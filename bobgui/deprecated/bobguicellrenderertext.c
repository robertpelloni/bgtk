 ...
 *
 * A `BobguiCellRendererText` renders a given text in its cell, using the font, color and
 * style information provided by its properties. The text will be ellipsized if it is
 * too long and the `BobguiCellRendererText:ellipsize` property allows it.
 *
 * If the `BobguiCellRenderer:mode` is %BOBGUI_CELL_RENDERER_MODE_EDITABLE,
 * the `BobguiCellRendererText` allows to edit its text using an entry.
 *
 * Deprecated: 4.10: List views use widgets to display their contents.
 *   You should use [class@Bobgui.Inscription] or [class@Bobgui.Label] instead
 */


static void bobgui_cell_renderer_text_finalize   (GObject                  *object);

static void bobgui_cell_renderer_text_get_property  (GObject                  *object,
						  guint                     param_id,
						  GValue                   *value,
						  GParamSpec               *pspec);
static void bobgui_cell_renderer_text_set_property  (GObject                  *object,
						  guint                     param_id,
						  const GValue             *value,
						  GParamSpec               *pspec);
static void bobgui_cell_renderer_text_snapshot   (BobguiCellRenderer          *cell,
...
static BobguiCellEditable *bobgui_cell_renderer_text_start_editing (BobguiCellRenderer      *cell,
							      GdkEvent             *event,
							      BobguiWidget            *widget,
							      const char           *path,
							      const GdkRectangle   *background_area,
...
static void       bobgui_cell_renderer_text_get_preferred_width            (BobguiCellRenderer       *cell,
                                                                         BobguiWidget             *widget,
                                                                         int                   *minimal_size,
                                                                         int                   *natural_size);
static void       bobgui_cell_renderer_text_get_preferred_height           (BobguiCellRenderer       *cell,
                                                                         BobguiWidget             *widget,
                                                                         int                   *minimal_size,
                                                                         int                   *natural_size);
static void       bobgui_cell_renderer_text_get_preferred_height_for_width (BobguiCellRenderer       *cell,
                                                                         BobguiWidget             *widget,
                                                                         int                    width,
                                                                         int                   *minimum_height,
                                                                         int                   *natural_height);
static void       bobgui_cell_renderer_text_get_aligned_area               (BobguiCellRenderer       *cell,
									 BobguiWidget             *widget,
									 BobguiCellRendererState   flags,
									 const GdkRectangle    *cell_area,
...
#define BOBGUI_CELL_RENDERER_TEXT_PATH "bobgui-cell-renderer-text-path"

typedef struct _BobguiCellRendererTextPrivate       BobguiCellRendererTextPrivate;

struct _BobguiCellRendererTextPrivate
{
  BobguiWidget *entry;
...
G_DEFINE_TYPE_WITH_PRIVATE (BobguiCellRendererText, bobgui_cell_renderer_text, BOBGUI_TYPE_CELL_RENDERER)

static void
bobgui_cell_renderer_text_init (BobguiCellRendererText *celltext)
{
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (celltext);
  BobguiCellRenderer *cell = BOBGUI_CELL_RENDERER (celltext);

  bobgui_cell_renderer_set_alignment (cell, 0.0, 0.5);
  bobgui_cell_renderer_set_padding (cell, 2, 2);
  priv->font_scale = 1.0;
...
static void
bobgui_cell_renderer_text_class_init (BobguiCellRendererTextClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);
  BobguiCellRendererClass *cell_class = BOBGUI_CELL_RENDERER_CLASS (class);

  object_class->finalize = bobgui_cell_renderer_text_finalize;

  object_class->get_property = bobgui_cell_renderer_text_get_property;
  object_class->set_property = bobgui_cell_renderer_text_set_property;

  cell_class->snapshot = bobgui_cell_renderer_text_snapshot;
  cell_class->start_editing = bobgui_cell_renderer_text_start_editing;
  cell_class->get_preferred_width = bobgui_cell_renderer_text_get_preferred_width;
  cell_class->get_preferred_height = bobgui_cell_renderer_text_get_preferred_height;
  cell_class->get_preferred_height_for_width = bobgui_cell_renderer_text_get_preferred_height_for_width;
  cell_class->get_aligned_area = bobgui_cell_renderer_text_get_aligned_area;
  ...
  /**
   * BobguiCellRendererText:background-rgba:
   *
  ...
  /**
   * BobguiCellRendererText:foreground-rgba:
   *
  ...
  /**
   * BobguiCellRendererText:ellipsize:
   *
  ...
  /**
   * BobguiCellRendererText:width-chars:
   *
  ...
  /**
   * BobguiCellRendererText:max-width-chars:
   *
   ...
   * set to expand in its `BobguiCellLayout` and all of the cell's siblings
  ...
  /**
   * BobguiCellRendererText:wrap-mode:
   *
  ...
  /**
   * BobguiCellRendererText:wrap-width:
   *
  ...
  /**
   * BobguiCellRendererText:alignment:
   *
   * Specifies how to align the lines of text with respect to each other.
   *
   * Note that this property describes how to align the lines of text in
   * case there are several of them. The "xalign" property of `BobguiCellRenderer`,
  ...
  /**
   * BobguiCellRendererText:placeholder-text:
   *
   * The text that will be displayed in the `BobguiCellRenderer` if
   * `BobguiCellRendererText:editable` is %TRUE and the cell is empty.
   */
...
#define ADD_SET_PROP(propname, propval, nick, blurb) text_cell_renderer_props[propval] = g_param_spec_boolean (propname, nick, blurb, FALSE, BOBGUI_PARAM_READWRITE)
  ...
  text_cell_renderer_signals [EDITED] =
    g_signal_new (I_("edited"),
		  ...
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiCellRendererTextClass, edited),
		  NULL, NULL,
...
static void
bobgui_cell_renderer_text_finalize (GObject *object)
{
  BobguiCellRendererText *celltext = BOBGUI_CELL_RENDERER_TEXT (object);
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (celltext);
  ...
  G_OBJECT_CLASS (bobgui_cell_renderer_text_parent_class)->finalize (object);
}
...
static void
bobgui_cell_renderer_text_get_property (GObject        *object,
				     guint           param_id,
				     GValue         *value,
				     GParamSpec     *pspec)
{
...
static void
set_bg_color (BobguiCellRendererText *celltext,
              GdkRGBA             *rgba)
{
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (celltext);
...
static void
set_fg_color (BobguiCellRendererText *celltext,
              GdkRGBA             *rgba)
{
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (celltext);
...
static void
set_font_description (BobguiCellRendererText  *celltext,
...
static void
bobgui_cell_renderer_text_set_property (GObject      *object,
				     guint         param_id,
				     const GValue *value,
				     GParamSpec   *pspec)
{
  ...
  switch (param_id)
    ...
    case PROP_EDITABLE:
      ...
      if (priv->editable)
        g_object_set (celltext, "mode", BOBGUI_CELL_RENDERER_MODE_EDITABLE, NULL);
      else
        g_object_set (celltext, "mode", BOBGUI_CELL_RENDERER_MODE_INERT, NULL);
 ...
 *
 * Creates a new `BobguiCellRendererText`. Adjust how text is drawn using
 ...
 * you can bind the â€œtextâ€ property on the cell renderer to a string
 ...
 **/
BobguiCellRenderer *
bobgui_cell_renderer_text_new (void)
{
  return g_object_new (BOBGUI_TYPE_CELL_RENDERER_TEXT, NULL);
}

static inline gboolean
show_placeholder_text (BobguiCellRendererText *celltext)
{
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (celltext);
...
static PangoLayout*
get_layout (BobguiCellRendererText *celltext,
            BobguiWidget           *widget,
            const GdkRectangle  *cell_area,
            BobguiCellRendererState flags)
{
  ...
  layout = bobgui_widget_create_pango_layout (widget, placeholder_layout ?
                                           priv->placeholder_text : priv->text);

  bobgui_cell_renderer_get_padding (BOBGUI_CELL_RENDERER (celltext), &xpad, NULL);
  ...
  if (!placeholder_layout && cell_area)
    {
      ...
      if (priv->foreground_set
	  && (flags & BOBGUI_CELL_RENDERER_SELECTED) == 0)
        {
          PangoColor color;
          guint16 alpha;
  ...
  else if (placeholder_layout)
    {
      ...
      bobgui_style_context_lookup_color (context, "placeholder_text_color", &fg);
  ...
  if ((flags & BOBGUI_CELL_RENDERER_PRELIT) == BOBGUI_CELL_RENDERER_PRELIT)
    {
      switch (uline)
        {
...
static void
get_size (BobguiCellRenderer    *cell,
	  BobguiWidget          *widget,
	  const GdkRectangle *cell_area,
	  PangoLayout        *layout,
	  int                *x_offset,
  ...
  if (priv->calc_fixed_height)
    {
      ...
      font_desc = bobgui_css_style_get_pango_font (bobgui_style_context_lookup_style (style_context));
	...
	pango_font_description_set_size (font_desc,
      ...
      bobgui_cell_renderer_get_fixed_size (cell, &cell_width, &cell_height);

      bobgui_cell_renderer_set_fixed_size (cell,
					cell_width, 2 * ypad +
...
static void
bobgui_cell_renderer_text_snapshot (BobguiCellRenderer      *cell,
			         BobguiSnapshot          *snapshot,
                                 BobguiWidget            *widget,
                                 const GdkRectangle   *background_area,
                                 const GdkRectangle   *cell_area,
...

{
  ...
  if (priv->background_set && (flags & BOBGUI_CELL_RENDERER_SELECTED) == 0)
    {
...
static void
bobgui_cell_renderer_text_editing_done (BobguiCellEditable *entry,
				     gpointer         data)
{
  BobguiCellRendererText *celltext = BOBGUI_CELL_RENDERER_TEXT (data);
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (celltext);
  const char *path;
  const char *new_text;
  gboolean canceled;
  ...
  path = g_object_get_data (G_OBJECT (entry), BOBGUI_CELL_RENDERER_TEXT_PATH);
  new_text = bobgui_editable_get_text (BOBGUI_EDITABLE (entry));
  g_signal_emit (data, text_cell_renderer_signals[EDITED], 0, path, new_text);
}

static void
bobgui_cell_renderer_text_focus_changed (BobguiWidget  *entry,
                                      GParamSpec *pspec,
                                      gpointer    data)
{
  if (bobgui_widget_has_focus (entry) ||
      bobgui_widget_has_focus (BOBGUI_WIDGET (bobgui_entry_get_text_widget (BOBGUI_ENTRY (entry)))))
    return;
...
bobgui_cell_renderer_text_start_editing (BobguiCellRenderer      *cell,
				      GdkEvent             *event,
				      BobguiWidget            *widget,
				      const char           *path,
				      const GdkRectangle   *background_area,
				      ...
				      BobguiCellRendererState  flags)
{
  ...
  bobgui_entry_set_has_frame (BOBGUI_ENTRY (priv->entry), FALSE);
  bobgui_entry_set_alignment (BOBGUI_ENTRY (priv->entry), xalign);
  bobgui_editable_set_width_chars (BOBGUI_EDITABLE (priv->entry), 5);

  if (priv->text)
    bobgui_editable_set_text (BOBGUI_EDITABLE (priv->entry), priv->text);
  g_object_set_data_full (G_OBJECT (priv->entry), I_(BOBGUI_CELL_RENDERER_TEXT_PATH), g_strdup (path), g_free);

  bobgui_editable_select_region (BOBGUI_EDITABLE (priv->entry), 0, -1);
  ...
  g_signal_connect (priv->entry, "editing-done",
		    G_CALLBACK (bobgui_cell_renderer_text_editing_done), celltext);
  priv->focus_out_id = g_signal_connect_after (priv->entry, "notify::has-focus",
					       G_CALLBACK (bobgui_cell_renderer_text_focus_changed),
					       celltext);

  return BOBGUI_CELL_EDITABLE (priv->entry);
}

/**
 * bobgui_cell_renderer_text_set_fixed_height_from_font:
 * @renderer: A `BobguiCellRendererText`
 * @number_of_rows: Number of rows of text each cell renderer is allocated, or -1
 *
 * Sets the height of a renderer to explicitly be determined by the â€œfontâ€ and
 * â€œy_padâ€ property set on it.  Further changes in these properties do not
 ...
 **/
void
bobgui_cell_renderer_text_set_fixed_height_from_font (BobguiCellRendererText *renderer,
						   int                  number_of_rows)
{
  BobguiCellRendererTextPrivate *priv = bobgui_cell_renderer_text_get_instance_private (renderer);
  BobguiCellRenderer *cell = BOBGUI_CELL_RENDERER (renderer);

  g_return_if_fail (BOBGUI_IS_CELL_RENDERER_TEXT (renderer));
  ...
  if (number_of_rows == -1)
    {
      ...
      bobgui_cell_renderer_get_fixed_size (cell, &width, &height);
      bobgui_cell_renderer_set_fixed_size (cell, width, -1);
    }
  else
    {
...
static void
bobgui_cell_renderer_text_get_preferred_width (BobguiCellRenderer *cell,
                                            BobguiWidget       *widget,
                                            int             *minimum_size,
                                            int             *natural_size)
{
   ...
   */
  bobgui_cell_renderer_get_padding (cell, &xpad, NULL);
...
static void
bobgui_cell_renderer_text_get_preferred_height_for_width (BobguiCellRenderer *cell,
                                                       BobguiWidget       *widget,
                                                       int              width,
                                                       int             *minimum_height,
                                                       int             *natural_height)
...
static void
bobgui_cell_renderer_text_get_preferred_height (BobguiCellRenderer *cell,
                                             BobguiWidget       *widget,
                                             int             *minimum_size,
                                             int             *natural_size)
{
   ...
   *
   * Note this code path won't be followed by BobguiTreeView which is
   * height-for-width specifically.
   */
  bobgui_cell_renderer_get_preferred_width (cell, widget, &min_width, NULL);
  bobgui_cell_renderer_text_get_preferred_height_for_width (cell, widget, min_width,
                                                         minimum_size, natural_size);
}

static void
bobgui_cell_renderer_text_get_aligned_area (BobguiCellRenderer       *cell,
					 BobguiWidget             *widget,
					 BobguiCellRendererState   flags,
					 const GdkRectangle    *cell_area,
					 GdkRectangle          *aligned_area)

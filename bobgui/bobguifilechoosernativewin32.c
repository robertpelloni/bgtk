  ...
  if (!data->skip_response)
    {
      ...
      _bobgui_native_dialog_emit_response (BOBGUI_NATIVE_DIALOG (data->self),
                                        data->response);
    }
  ...
  if (data->filters)
    {
      ...
      if (data->self->current_filter)
        {
          GListModel *filters;
          guint current_filter_index = BOBGUI_INVALID_LIST_POSITION;

          filters = bobgui_file_chooser_get_filters (BOBGUI_FILE_CHOOSER (data->self));
...
static gboolean
file_filter_to_win32 (BobguiFileFilter *filter,
                      COMDLG_FILTERSPEC *spec)
{
  ...
  patterns = _bobgui_file_filter_get_as_patterns (filter);
  if (patterns == NULL)
    return FALSE;
  ...
  name = bobgui_file_filter_get_name (filter);
  if (name == NULL)
    name = pattern_list;
...
gboolean
bobgui_file_chooser_native_win32_show (BobguiFileChooserNative *self)
{
  GThread *thread;
  FilechooserWin32ThreadData *data;
  ...
  filters = bobgui_file_chooser_get_filters (BOBGUI_FILE_CHOOSER (self));
  ...
  data->shortcut_files =
    bobgui_file_chooser_get_shortcut_folders (BOBGUI_FILE_CHOOSER (self->dialog));
  ...
  if (action == BOBGUI_FILE_CHOOSER_ACTION_SAVE)
    data->save = TRUE;

  if (action == BOBGUI_FILE_CHOOSER_ACTION_SELECT_FOLDER)
    data->folder = TRUE;

  if ((action == BOBGUI_FILE_CHOOSER_ACTION_SELECT_FOLDER ||
       action == BOBGUI_FILE_CHOOSER_ACTION_OPEN) &&
      bobgui_file_chooser_get_select_multiple (BOBGUI_FILE_CHOOSER (self->dialog)))
    data->select_multiple = TRUE;

  transient_for = bobgui_native_dialog_get_transient_for (BOBGUI_NATIVE_DIALOG (self));
  if (transient_for)
    {
      bobgui_widget_realize (BOBGUI_WIDGET (transient_for));
      data->parent = gdk_win32_surface_get_handle (bobgui_native_get_surface (BOBGUI_NATIVE (transient_for)));

      if (bobgui_native_dialog_get_modal (BOBGUI_NATIVE_DIALOG (self)))
        data->modal = TRUE;
    }

  data->title =
    g_strdup (bobgui_native_dialog_get_title (BOBGUI_NATIVE_DIALOG (self)));
...
void
bobgui_file_chooser_native_win32_hide (BobguiFileChooserNative *self)
{

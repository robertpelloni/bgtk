 ...
 * BobguiTextTag:
 *
 * Can be applied to text contained in a `BobguiTextBuffer`.
 *
 ...
 *
 * Tags should be in the [class@Bobgui.TextTagTable] for a given
 * `BobguiTextBuffer` before using them with that buffer.
 *
 * [method@Bobgui.TextBuffer.create_tag] is the best way to create tags.
 * See â€œbobgui4-demoâ€ for numerous examples.
 *
 * For each property of `BobguiTextTag`, there is a â€œsetâ€ property, e.g.
 * â€œfont-setâ€ corresponds to â€œfontâ€. These â€œsetâ€ properties reflect
 * whether a property has been set or not.
 *
 * They are maintained by BOBGUI and you should not set them independently.
 */
...
static void bobgui_text_tag_finalize     (GObject         *object);
static void bobgui_text_tag_set_property (GObject         *object,
                                       guint            prop_id,
                                       const GValue    *value,
                                       GParamSpec      *pspec);
static void bobgui_text_tag_get_property (GObject         *object,
                                       guint            prop_id,
                                       GValue          *value,
                                       GParamSpec      *pspec);

G_DEFINE_TYPE_WITH_PRIVATE (BobguiTextTag, bobgui_text_tag, G_TYPE_OBJECT)

static void
bobgui_text_tag_class_init (BobguiTextTagClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = bobgui_text_tag_set_property;
  object_class->get_property = bobgui_text_tag_get_property;
  
  object_class->finalize = bobgui_text_tag_finalize;
  ...
  /**
   * BobguiTextTag:background-full-height:
   *
   ...
   * on the current locale, see also [func@Bobgui.get_default_language].
   */
  g_object_class_install_property (object_class,
                                   PROP_LANGUAGE,
  ...
  /**
   * BobguiTextTag:pixels-above-lines:
   *
  ...
  /**
   * BobguiTextTag:pixels-below-lines:
   *
  ...
  /**
   * BobguiTextTag:pixels-inside-wrap:
   *
  ...
  /**
   * BobguiTextTag:underline-rgba:
   *
   * This property modifies the color of underlines.
   *
   * If not set, underlines will use the foreground color.
   *
   * If [property@Bobgui.TextTag:underline] is set to %PANGO_UNDERLINE_ERROR,
  ...
  /**
   * BobguiTextTag:strikethrough-rgba:
   *
  ...
  /**
   * BobguiTextTag:paragraph-background:
   *
  ...
  /**
   * BobguiTextTag:paragraph-background-rgba:
   *
  ...
  /**
   * BobguiTextTag:accumulative-margin:
   *
...
#define ADD_SET_PROP(propname, propval, nick, blurb) g_object_class_install_property (object_class, propval, g_param_spec_boolean (propname, nick, blurb, FALSE, BOBGUI_PARAM_READWRITE))

  /**
   * BobguiTextTag:background-set:
   *
  ...
  /**
   * BobguiTextTag:background-full-height-set:
   *
  ...
  /**
   * BobguiTextTag:justification-set:
   *
  ...
  /**
   * BobguiTextTag:pixels-above-lines-set:
   *
  ...
  /**
   * BobguiTextTag:pixels-below-lines-set:
   *
  ...
  /**
   * BobguiTextTag:pixels-inside-wrap-set:
   *
  ...
  /**
   * BobguiTextTag:strikethrough-set:
   *
  ...
  /**
   * BobguiTextTag:right-margin-set:
   *
  ...
  /**
   * BobguiTextTag:underline-rgba-set:
   *
  ...
  /**
   * BobguiTextTag:overline-rgba-set:
   *
  ...
  /**
   * BobguiTextTag:strikethrough-rgba-set:
   *
  ...
  /**
   * BobguiTextTag:paragraph-background-set:
   *
  ...
  /**
   * BobguiTextTag:letter-spacing-set:
   *
  ...
  /**
   * BobguiTextTag:font-features-set:
   *
  ...
  /**
   * BobguiTextTag:allow-breaks-set:
   *
  ...
  /**
   * BobguiTextTag:insert-hyphens-set:
   *
  ...
  /**
   * BobguiTextTag:text-transform-set:
   *
...
static void
bobgui_text_tag_init (BobguiTextTag *text_tag)
{
  text_tag->priv = bobgui_text_tag_get_instance_private (text_tag);
  text_tag->priv->values = bobgui_text_attributes_new ();
}
 ...
 *
 * Creates a `BobguiTextTag`.
 *
 * Returns: a new `BobguiTextTag`
 */
BobguiTextTag*
bobgui_text_tag_new (const char *name)
{
  BobguiTextTag *tag;

  tag = g_object_new (BOBGUI_TYPE_TEXT_TAG, "name", name, NULL);

  return tag;
}

static void
bobgui_text_tag_finalize (GObject *object)
{
  BobguiTextTag *text_tag = BOBGUI_TEXT_TAG (object);
  BobguiTextTagPrivate *priv = text_tag->priv;

  if (priv->table)
    bobgui_text_tag_table_remove (priv->table, text_tag);

  g_assert (priv->table == NULL);

  bobgui_text_attributes_unref (priv->values);
  priv->values = NULL;

  g_free (priv->name);
  priv->name = NULL;

  G_OBJECT_CLASS (bobgui_text_tag_parent_class)->finalize (object);
}

static void
set_underline_rgba (BobguiTextTag    *tag,
                    const GdkRGBA *rgba)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
static void
set_overline_rgba (BobguiTextTag    *tag,
                   const GdkRGBA *rgba)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
static void
set_strikethrough_rgba (BobguiTextTag    *tag,
                        const GdkRGBA *rgba)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
static void
set_bg_rgba (BobguiTextTag *tag, GdkRGBA *rgba)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
static void
set_fg_rgba (BobguiTextTag *tag, GdkRGBA *rgba)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
static void
set_pg_bg_rgba (BobguiTextTag *tag, GdkRGBA *rgba)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
static void
set_font_description (BobguiTextTag           *text_tag,
...
static void
bobgui_text_tag_ensure_font (BobguiTextTag *text_tag)
{
  BobguiTextTagPrivate *priv = text_tag->priv;
...
static void
bobgui_text_tag_set_property (GObject      *object,
                           guint         prop_id,
                           const GValue *value,
                           GParamSpec   *pspec)
{
  ...
  switch (prop_id)
	...
	bobgui_text_tag_ensure_font (text_tag);
   ...
   * signal is thawed.
   */
  bobgui_text_tag_changed (text_tag, size_changed);
}

static void
bobgui_text_tag_get_property (GObject      *object,
                           guint         prop_id,
                           GValue       *value,
                           GParamSpec   *pspec)
{
  BobguiTextTag *tag = BOBGUI_TEXT_TAG (object);
  BobguiTextTagPrivate *priv = tag->priv;
  ...
  switch (prop_id)
    ...
    case PROP_FONT:
	  ...
	  bobgui_text_tag_ensure_font (tag);
...
static void
delta_priority_foreach (BobguiTextTag *tag, gpointer user_data)
{
  BobguiTextTagPrivate *priv = tag->priv;
...
/**
 * bobgui_text_tag_get_priority:
 * @tag: a `BobguiTextTag`
 *
 * Get the tag priority.
 *
 * Returns: The tagâ€™s priority.
 */
int
bobgui_text_tag_get_priority (BobguiTextTag *tag)
{
  g_return_val_if_fail (BOBGUI_IS_TEXT_TAG (tag), 0);
 ...
 *
 * Sets the priority of a `BobguiTextTag`.
 *
 * Valid priorities start at 0 and go to one less than
 * [method@Bobgui.TextTagTable.get_size]. Each tag in a table
 ...
 *
 * Higher priority tags â€œwinâ€ if two tags both set the same text
 ...
 * to the table, or created with [method@Bobgui.TextBuffer.create_tag],
 * which adds the tag to the bufferâ€™s table automatically.
 */
void
bobgui_text_tag_set_priority (BobguiTextTag *tag,
                           int         priority)
{
  ...
  g_return_if_fail (priority < bobgui_text_tag_table_get_size (priv->table));
  ...
  bobgui_text_tag_table_foreach (priv->table,
...
/**
 * bobgui_text_tag_changed:
 * @tag: a `BobguiTextTag`
 * @size_changed: whether the change affects the `BobguiTextView` layout
 *
 * Emits the [signal@Bobgui.TextTagTable::tag-changed] signal on the
 * `BobguiTextTagTable` where the tag is included.
 *
 * The signal is already emitted when setting a `BobguiTextTag` property.
 * This function is useful for a `BobguiTextTag` subclass.
 */
void
bobgui_text_tag_changed (BobguiTextTag *tag,
                      gboolean    size_changed)
{
   ...
   * two objects are already tightly bound. If a BobguiTextTag::changed signal is
...
{
  BobguiTextTag *tag1, *tag2;

  tag1 = * (BobguiTextTag **) first;
  tag2 = * (BobguiTextTag **) second;
  return tag1->priv->priority - tag2->priv->priority;
}

void
_bobgui_text_tag_array_sort (GPtrArray *tags)
{

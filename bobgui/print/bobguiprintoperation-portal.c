                         ...
                         char            **token)
{
  ...
  *token = g_strdup_printf ("bobgui%d", g_random_int_range (0, G_MAXINT));
...
portal_data_free (gpointer data)
{
  ...
  if (portal->parent && portal->handle)
    bobgui_window_unexport_handle (portal->parent, portal->handle);
...
} BobguiPrintOperationPortal;

static void
op_portal_free (BobguiPrintOperationPortal *op_portal)
{
  BobguiPrintBackend *backend;

  backend = bobgui_printer_get_backend (op_portal->printer);
...
static void
portal_start_page (BobguiPrintOperation *op,
                   BobguiPrintContext   *print_context,
                   BobguiPageSetup      *page_setup)
{
  BobguiPrintOperationPortal *op_portal = op->priv->platform_data;
...
static void
portal_end_page (BobguiPrintOperation *op,
                 BobguiPrintContext   *print_context)
{
  cairo_t *cr;
                 ...
                 gpointer data)
{
  ...
  // TODO: Delay BOBGUI_PRINT_STATUS_FINISHED to org.freedesktop.portal.Request::Response ?
  _bobgui_print_operation_set_status (op,
                                   ret == NULL ? BOBGUI_PRINT_STATUS_FINISHED_ABORTED :
...
static void
portal_job_complete (BobguiPrintJob  *job,
                     gpointer      data,
                     const GError *error)
{
  BobguiPrintOperation *op = data;
  ...
  _bobgui_print_operation_set_status (op, BOBGUI_PRINT_STATUS_SENDING_DATA, NULL);

  settings = bobgui_print_job_get_settings (job);
  uri = bobgui_print_settings_get (settings, BOBGUI_PRINT_SETTINGS_OUTPUT_URI);
...
static void
portal_end_run (BobguiPrintOperation *op,
                gboolean           wait,
                gboolean           cancelled)
{
  BobguiPrintOperationPortal *op_portal = op->priv->platform_data;
  ...
  if (op_portal->job != NULL)
    {
      g_object_ref (op);
      bobgui_print_job_send (op_portal->job, portal_job_complete, op, NULL);
    }
...
{
  BobguiPrintOperation *op = portal->op;
  BobguiPrintOperationPrivate *priv = op->priv;
  BobguiPrintJob *job;
  BobguiPrintOperationPortal *op_portal;
  cairo_t *cr;
  ...
  if (portal->do_print)
    {
      ...
      op_portal = g_new0 (BobguiPrintOperationPortal, 1);
      ...
      op_portal->surface = bobgui_print_job_get_surface (job, &priv->error);
      if (op_portal->surface == NULL)
        {
          portal->result = BOBGUI_PRINT_OPERATION_RESULT_ERROR;
...
find_file_printer (void)
{
  ...
  backends = bobgui_print_backend_load_modules ();
  for (l = backends; l; l = l->next)
    {
      BobguiPrintBackend *backend = l->data;

      /* FIXME: this needs changes for cpdb */
      if (!printer && strcmp (G_OBJECT_TYPE_NAME (backend), "BobguiPrintBackendFileBuiltin") == 0)
        {
          printers = bobgui_print_backend_get_printer_list (backend);
      ...
      if (printer)
        {
          ...
          int fd;
          char *uri;
          gboolean is_file_printer = !!bobgui_print_settings_get(settings, BOBGUI_PRINT_SETTINGS_OUTPUT_URI);

          fd = g_file_open_tmp ("bobguiprintXXXXXX", &filename, NULL);
          uri = g_filename_to_uri (filename, NULL, NULL);
          bobgui_print_settings_set (settings, BOBGUI_PRINT_SETTINGS_OUTPUT_URI, uri);
          g_free (uri);
          close (fd);

          portal->result = BOBGUI_PRINT_OPERATION_RESULT_APPLY;
...
static PortalData *
create_portal_data (BobguiPrintOperation          *op,
                    BobguiWindow                  *parent,
                    BobguiPrintOperationPrintFunc  print_cb)
{
  GDBusProxy *proxy;
  ...
  signal_id = g_signal_lookup ("create-custom-widget", BOBGUI_TYPE_PRINT_OPERATION);
  if (g_signal_has_handler_pending (op, signal_id, 0, TRUE))
    g_warning ("BobguiPrintOperation::create-custom-widget not supported with portal");
...
static void
window_handle_exported (BobguiWindow  *window,
...
static void
call_prepare_print (BobguiPrintOperation *op,
                    PortalData        *portal)
{
  BobguiPrintOperationPrivate *priv = op->priv;
  GVariantBuilder opt_builder;
  ...
  if (priv->print_settings)
    portal->settings = bobgui_print_settings_to_gvariant (priv->print_settings);
  else
    {
  ...
  if (priv->default_page_setup)
    portal->setup = bobgui_page_setup_to_gvariant (priv->default_page_setup);
  else
    {
      BobguiPageSetup *page_setup = bobgui_page_setup_new ();
      portal->setup = bobgui_page_setup_to_gvariant (page_setup);
  ...
  if (portal->parent != NULL &&
      bobgui_widget_is_visible (BOBGUI_WIDGET (portal->parent)) &&
      bobgui_window_export_handle (portal->parent, window_handle_exported, portal))
    return;
...
bobgui_print_operation_portal_run_dialog (BobguiPrintOperation *op,
                                       ...
                                       gboolean          *do_print)
{
  ...
  if (portal == NULL)
    return BOBGUI_PRINT_OPERATION_RESULT_ERROR;
...
void
bobgui_print_operation_portal_run_dialog_async (BobguiPrintOperation          *op,
                                             gboolean                    show_dialog,
                                             BobguiWindow                  *parent,
                                             BobguiPrintOperationPrintFunc  print_cb)
{
...
void
bobgui_print_operation_portal_launch_preview (BobguiPrintOperation *op,
                                           cairo_surface_t   *surface,
                                           BobguiWindow         *parent,
                                           const char        *filename)
{

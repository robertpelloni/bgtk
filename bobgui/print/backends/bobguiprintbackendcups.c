 ...
 * bobguiprintbackendcups.h: Default implementation of BobguiPrintBackend
...
#include "bobguiprintbackendcups.h"
#include "bobguiprintercups.h"

#include "bobguicupsutils.h"
#include "bobguicupssecretsutils.h"
#include "bobguiprintbackendutils.h"

#include <bobgui/print/bobguiprintutilsprivate.h>
...
#define BOBGUI_PRINT_BACKEND_CUPS_GET_CLASS(obj)   (G_TYPE_INSTANCE_GET_CLASS ((obj), BOBGUI_TYPE_PRINT_BACKEND_CUPS, BobguiPrintBackendCupsClass))
...
typedef void (* BobguiPrintCupsResponseCallbackFunc) (BobguiPrintBackend *print_backend,
...
} BobguiPrintCupsDispatchWatch;

struct _BobguiPrintBackendCupsClass
{
  BobguiPrintBackendClass parent_class;
};

struct _BobguiPrintBackendCups
{
  ...
  BobguiCupsConnectionTest *cups_connection_test;
  int   reading_ppds;
...
static void                 bobgui_print_backend_cups_finalize        (GObject                           *object);
static void                 bobgui_print_backend_cups_dispose         (GObject                           *object);
static void                 cups_get_printer_list                  (BobguiPrintBackend                   *print_backend);
static void                 cups_get_default_printer               (BobguiPrintBackendCups               *print_backend);
static void                 cups_get_local_default_printer         (BobguiPrintBackendCups               *print_backend);
static void                 cups_request_execute                   (BobguiPrintBackendCups               *print_backend,
								    BobguiCupsRequest                    *request,
								    BobguiPrintCupsResponseCallbackFunc   callback,
								    gpointer                           user_data,
...
static gboolean             cups_printer_mark_conflicts            (BobguiPrinter                        *printer,
								    BobguiPrinterOptionSet               *options);
static BobguiPrinterOptionSet *cups_printer_get_options               (BobguiPrinter                        *printer,
								    BobguiPrintSettings                  *settings,
								    BobguiPageSetup                      *page_setup,
                                                                    BobguiPrintCapabilities               capabilities);
static void                 cups_printer_prepare_for_print         (BobguiPrinter                        *printer,
								    BobguiPrintJob                       *print_job,
								    BobguiPrintSettings                  *settings,
								    BobguiPageSetup                      *page_setup);
static GList *              cups_printer_list_papers               (BobguiPrinter                        *printer);
static BobguiPageSetup *       cups_printer_get_default_page_size     (BobguiPrinter                        *printer);
static void                 cups_printer_request_details           (BobguiPrinter                        *printer);
static gboolean             cups_request_default_printer           (BobguiPrintBackendCups               *print_backend);
#if CUPS_VERSION_MAJOR < 3
static gboolean             cups_request_ppd                       (BobguiPrinter                        *printer);
#endif
static gboolean             cups_printer_get_hard_margins          (BobguiPrinter                        *printer,
								    double                            *top,
								    double                            *bottom,
								    double                            *left,
								    double                            *right);
static gboolean             cups_printer_get_hard_margins_for_paper_size (BobguiPrinter                  *printer,
									  BobguiPaperSize                *paper_size,
									  double                      *top,
									  double                      *bottom,
									  double                      *left,
									  double                      *right);
static BobguiPrintCapabilities cups_printer_get_capabilities          (BobguiPrinter                        *printer);
static void                 set_option_from_settings               (BobguiPrinterOption                  *option,
...
static cairo_surface_t *    cups_printer_create_cairo_surface      (BobguiPrinter                        *printer,
								    BobguiPrintSettings                  *settings,
								    double                             width,
								    double                             height,
								    GIOChannel                        *cache_io);

static void                 bobgui_print_backend_cups_set_password    (BobguiPrintBackend                   *backend,
...
/**
 * bobgui_print_backend_cups_new:
 *
 * Creates a new #BobguiPrintBackendCups object. #BobguiPrintBackendCups
 * implements the #BobguiPrintBackend interface with direct access to
 * the filesystem using Unix/Linux API calls
 *
 * Returns: the new #BobguiPrintBackendCups object
 */
BobguiPrintBackend *
bobgui_print_backend_cups_new (void)
{
  BOBGUI_DEBUG (PRINTING, "CUPS Backend: Creating a new CUPS print backend object");

  return g_object_new (BOBGUI_TYPE_PRINT_BACKEND_CUPS, NULL);
}

static void                 create_temporary_queue                  (BobguiPrintBackendCups *backend,
                                                                     const gchar         *printer_name,
                                                                     const gchar         *printer_uri,
                                                                     const gchar         *device_uri);

static void
bobgui_print_backend_cups_class_init (BobguiPrintBackendCupsClass *class)
{
  ...
  gobject_class->finalize = bobgui_print_backend_cups_finalize;
  gobject_class->dispose = bobgui_print_backend_cups_dispose;

  backend_class->request_printer_list = cups_get_printer_list;
  backend_class->print_stream = bobgui_print_backend_cups_print_stream;
  ...
  backend_class->set_password = bobgui_print_backend_cups_set_password;
}

static gboolean
option_is_ipp_option (BobguiPrinterOption *option)
{
...
static void
option_set_is_ipp_option (BobguiPrinterOption *option,
                      ...
                      unsigned int         length)
{
  ...
  BOBGUI_DEBUG (PRINTING, "CUPS Backend: Writing %i byte chunk to temp file", length);
  ...
  while (length > 0)
    {
      ...
      if (error != NULL)
	{
	  BOBGUI_DEBUG (PRINTING, "CUPS Backend: Error writing to temp file, %s",
                             error->message);
      ...
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Wrote %"G_GSIZE_FORMAT" bytes to temp file",
                           written);
...
cups_printer_create_cairo_surface (BobguiPrinter       *printer,
				   BobguiPrintSettings *settings,
				   double            width,
				   double            height,
				   GIOChannel       *cache_io)
{
  ...
  if (bobgui_printer_accepts_pdf (printer))
...
#if CUPS_VERSION_MAJOR < 3
  ppd_file = bobgui_printer_cups_get_ppd (BOBGUI_PRINTER_CUPS (printer));
  ...
  if (ppd_file != NULL)
    {
      ...
      if (bobgui_print_settings_get_resolution (settings) == 0)
        {
          ...
          if (ppd_attr_res != NULL)
            {
              ...
              if (sscanf (ppd_attr_res->value, "%dx%ddpi", &res_x, &res_y) == 2)
                {
                  if (res_x > 0 && res_y > 0)
                    bobgui_print_settings_set_resolution_xy (settings, res_x, res_y);
                }
              else if (sscanf (ppd_attr_res->value, "%ddpi", &res) == 1)
                {
                  if (res > 0)
                    bobgui_print_settings_set_resolution (settings, res);
                }
            }
        }

      res_string = g_strdup_printf ("%ddpi",
                                    bobgui_print_settings_get_resolution (settings));
      ...
      if (ppd_attr_res_screen_freq == NULL)
        {
          res_string = g_strdup_printf ("%dx%ddpi",
                                        bobgui_print_settings_get_resolution_x (settings),
                                        bobgui_print_settings_get_resolution_y (settings));
          ppd_attr_res_screen_freq = ppdFindAttr (ppd_file, "ResScreenFreq", res_string);
          g_free (res_string);
        }
      ...
      if (ppd_attr_res_screen_freq != NULL && atof (ppd_attr_res_screen_freq->value) > 0.0)
        bobgui_print_settings_set_printer_lpi (settings, atof (ppd_attr_res_screen_freq->value));
      else if (ppd_attr_screen_freq != NULL && atof (ppd_attr_screen_freq->value) > 0.0)
        bobgui_print_settings_set_printer_lpi (settings, atof (ppd_attr_screen_freq->value));
    }
#endif
...
static void
cups_print_cb (BobguiPrintBackendCups *print_backend,
               BobguiCupsResult       *result,
               gpointer             user_data)
{
  GError *error = NULL;
  ...
  if (bobgui_cups_result_is_error (result))
    error = g_error_new_literal (bobgui_print_error_quark (),
                                 BOBGUI_PRINT_ERROR_INTERNAL_ERROR,
                                 bobgui_cups_result_get_error_string (result));
  ...
  if (error == NULL)
    {
      ...
      ipp_t *response = bobgui_cups_result_get_response (result);

      if ((attr = ippFindAttribute (response, "job-id", IPP_TAG_INTEGER)) != NULL)
	job_id = ippGetInteger (attr, 0);

      if (!bobgui_print_job_get_track_print_status (ps->job) || job_id == 0)
	bobgui_print_job_set_status (ps->job, BOBGUI_PRINT_STATUS_FINISHED);
      else
	{
	  bobgui_print_job_set_status (ps->job, BOBGUI_PRINT_STATUS_PENDING);
	  cups_begin_polling_info (print_backend, ps->job, job_id);
	}
    }
  else
    bobgui_print_job_set_status (ps->job, BOBGUI_PRINT_STATUS_FINISHED_ABORTED);
		  ...
		  gpointer     user_data)
{
  ...
  BobguiCupsRequest *request = data->request;
#if CUPS_VERSION_MAJOR < 3
  BobguiPrinterCups *printer = data->printer;
#endif
  ...
  if (strcmp (value, "bobgui-ignore-value") == 0)
    return;
  ...
  if (custom_value)
    ...
    bobgui_cups_request_encode_option (request, key, value);
}

static void
bobgui_print_backend_cups_print_stream (BobguiPrintBackend         *print_backend,
                                     BobguiPrintJob             *job,
				     GIOChannel              *data_io,
				     BobguiPrintJobCompleteFunc  callback,
				     gpointer                 user_data,
				     GDestroyNotify           dnotify)
{
  ...
  cups_printer = BOBGUI_PRINTER_CUPS (bobgui_print_job_get_printer (job));
  ...
  if (cups_printer->avahi_browsed)
      ...
      if (http)
        {
          request = bobgui_cups_request_new_with_username (http,
                                                        ...
                                                        BOBGUI_PRINT_BACKEND_CUPS (print_backend)->username);
      ...
      else
        {
          ...
          BOBGUI_DEBUG (PRINTING, "CUPS Backend: Error connecting to %s:%d",
                               cups_printer->hostname,
                               cups_printer->port);

          error = g_error_new (bobgui_print_error_quark (),
                               BOBGUI_CUPS_ERROR_GENERAL,
                               "Error connecting to %s",
                               cups_printer->hostname);

          bobgui_print_job_set_status (job, BOBGUI_PRINT_STATUS_FINISHED_ABORTED);
    ...
    {
      request = bobgui_cups_request_new_with_username (NULL,
                                                    BOBGUI_CUPS_POST,
                                                    IPP_OP_PRINT_JOB,
                                                    data_io,
                                                    NULL,
      ...
      httpAssembleURIf (HTTP_URI_CODING_ALL,
                        ...
                        ippGetPort (),
                        "/printers/%s",
                        bobgui_printer_get_name (bobgui_print_job_get_printer (job)));
    }

  bobgui_cups_request_set_ipp_version (request,
  ...
  if (cups_printer->auth_info_required != NULL &&
    ...
    {
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Ignoring auth-info-required \"%s\"",
  ...
  cups_request_execute (BOBGUI_PRINT_BACKEND_CUPS (print_backend),
                        request,
                        (BobguiPrintCupsResponseCallbackFunc) cups_print_cb,
                        ps,
...
static void
bobgui_print_backend_cups_init (BobguiPrintBackendCups *backend_cups)
{
  int i;
  ...
  backend_cups->secrets_service_watch_id =
    bobgui_cups_secrets_service_watch (secrets_service_appeared_cb,
...
static void
bobgui_print_backend_cups_finalize (GObject *object)
{
  BobguiPrintBackendCups *backend_cups;

  BOBGUI_DEBUG (PRINTING, "CUPS Backend: finalizing CUPS backend module");

  backend_cups = BOBGUI_PRINT_BACKEND_CUPS (object);

  g_free (backend_cups->default_printer);
  backend_cups->default_printer = NULL;

  bobgui_cups_connection_test_free (backend_cups->cups_connection_test);
  backend_cups->cups_connection_test = NULL;
...
static void
bobgui_print_backend_cups_set_password (BobguiPrintBackend  *backend,
                                     char            **auth_info_required,
                                     char            **auth_info,
                                     gboolean          store_auth_info)
{
  BobguiPrintBackendCups *cups_backend = BOBGUI_PRINT_BACKEND_CUPS (backend);
  GList *l;
  ...
  if (hostname != NULL && username != NULL && password != NULL)
    {
      ...
      BOBGUI_DEBUG (PRINTING, "CUPS backend: caching password for %s", key);
    }
  ...
  for (l = cups_backend->requests; l; l = l->next)
    {
      BobguiPrintCupsDispatchWatch *dispatch = l->data;
          ...
          if (password != NULL && store_auth_info)
            {
              const char *printer_uri =
                  bobgui_cups_request_ipp_get_string (dispatch->request,
                                                   IPP_TAG_URI,
                                                   "printer-uri");

              bobgui_cups_secrets_service_store (auth_info, auth_info_required,
                                              printer_uri);
            }
          dispatch->backend->authentication_lock = FALSE;
          dispatch->request->need_auth_info = FALSE;
        }
      else if (dispatch->request->password_state == BOBGUI_CUPS_PASSWORD_REQUESTED || auth_info == NULL)
        {
          ...
          dispatch->request->password_state = BOBGUI_CUPS_PASSWORD_HAS;
          dispatch->backend->authentication_lock = FALSE;
        }
    }
}
...
{
  BobguiPrintCupsDispatchWatch *dispatch = data;
  const char                *username;
  char                      *password;
  char                      *prompt = NULL;
  char                      *key = NULL;
  ...
  if (password && dispatch->request->password_state != BOBGUI_CUPS_PASSWORD_NOT_VALID)
    {
      BOBGUI_DEBUG (PRINTING, "CUPS backend: using stored password for %s", key);
    ...
    {
      const char *job_title = bobgui_cups_request_ipp_get_string (dispatch->request, IPP_TAG_NAME, "job-name");
      const char *printer_uri = bobgui_cups_request_ipp_get_string (dispatch->request, IPP_TAG_URI, "printer-uri");
      char *printer_name = NULL;

      if (printer_uri != NULL && strrchr (printer_uri, '/') != NULL)
        printer_name = g_strdup (strrchr (printer_uri, '/') + 1);

      if (dispatch->request->password_state == BOBGUI_CUPS_PASSWORD_NOT_VALID)
        g_hash_table_remove (dispatch->backend->auth, key);

      dispatch->request->password_state = BOBGUI_CUPS_PASSWORD_REQUESTED;
      ...
      switch ((guint)ippGetOperation (dispatch->request->ipp_request))
        {
          case IPP_OP_PRINT_JOB:
            if (job_title != NULL && printer_name != NULL)
              prompt = g_strdup_printf ( _("Authentication is required to print document â€œ%sâ€ on printer %s"), job_title, printer_name);
            else
              prompt = g_strdup_printf ( _("Authentication is required to print a document on %s"), hostname);
            break;
          case IPP_OP_GET_JOB_ATTRIBUTES:
            if (job_title != NULL)
              prompt = g_strdup_printf ( _("Authentication is required to get attributes of job â€œ%sâ€"), job_title);
            else
...
{
  BobguiPrintCupsDispatchWatch *dispatch;
  BobguiCupsPollState poll_state;

  dispatch = (BobguiPrintCupsDispatchWatch *) source;

  poll_state = bobgui_cups_request_get_poll_state (dispatch->request);
  ...
  if (dispatch->request->http != NULL)
    {
	  ...
	  if (poll_state == BOBGUI_CUPS_HTTP_READ)
	    dispatch->data_poll->events = G_IO_IN | G_IO_HUP | G_IO_ERR | G_IO_PRI;
	  else if (poll_state == BOBGUI_CUPS_HTTP_WRITE)
...
{
  BobguiPrintCupsDispatchWatch *dispatch;
  dispatch = (BobguiPrintCupsDispatchWatch *) user_data;

  if (!dispatch->request->need_auth_info)
    {
      if (dispatch->request->auth_info == NULL)
        {
          dispatch->callback (BOBGUI_PRINT_BACKEND (dispatch->backend),
                              bobgui_cups_request_get_result (dispatch->request),
      ...
      else
        {
          ...
          bobgui_cups_request_ipp_add_strings (dispatch->request,
                                            IPP_TAG_JOB,
                                            IPP_TAG_TEXT,
                                            "auth-info",
                                            length,
                     ...
                     gpointer      user_data)
{
  ...
  if (auth_info == NULL)
    {
      if (error != NULL)
        {
          BOBGUI_DEBUG (PRINTING, "Failed to look up auth info: %s", error->message);
          g_error_free (error);
        }
      else
        {
          /* Error note should have been shown by the function causing this */
          BOBGUI_DEBUG (PRINTING, "Failed to look up auth info.");
        }
      dispatch->backend->authentication_lock = FALSE;
      g_object_unref (task);
  ...
  bobgui_print_backend_cups_set_password (BOBGUI_PRINT_BACKEND (dispatch->backend),
...
lookup_auth_info (gpointer user_data)
{
  ...
  if (dispatch->backend->secrets_service_available && need_secret_auth_info)
    {
      dispatch->backend->authentication_lock = TRUE;
      printer_uri = bobgui_cups_request_ipp_get_string (dispatch->request,
                                                     IPP_TAG_URI,
                                                     "printer-uri");
      bobgui_cups_secrets_service_query_task (dispatch->backend,
...
request_auth_info (gpointer user_data)
{
  ...
  job_title = bobgui_cups_request_ipp_get_string (dispatch->request, IPP_TAG_NAME, "job-name");
  printer_uri = bobgui_cups_request_ipp_get_string (dispatch->request, IPP_TAG_URI, "printer-uri");
  length = g_strv_length (dispatch->request->auth_info_required);
  ...
  if (job_title != NULL)
    {
      if (printer_name != NULL)
        ...
        prompt = g_strdup_printf ( _("Authentication is required to print document â€œ%sâ€"), job_title);
    }
  else
    {
      if (printer_name != NULL)
...
cups_dispatch_watch_check (GSource *source)
{
  ...
  BOBGUI_DEBUG (PRINTING, "CUPS Backend: %s <source %p>", G_STRFUNC, source);

  dispatch = (BobguiPrintCupsDispatchWatch *) source;

  poll_state = bobgui_cups_request_get_poll_state (dispatch->request);

  if (poll_state != BOBGUI_CUPS_HTTP_IDLE && !dispatch->request->need_password)
    if (!(dispatch->data_poll->revents & dispatch->data_poll->events))
       return FALSE;

  result = bobgui_cups_request_read_write (dispatch->request, FALSE);
  ...
  if (dispatch->request->need_password && dispatch->request->password_state != BOBGUI_CUPS_PASSWORD_REQUESTED)
    {
			     ...
			     int     *timeout_)
{
  ...
  *timeout_ = -1;
  ...
  result = bobgui_cups_request_read_write (dispatch->request, TRUE);
...
{
  BobguiPrintCupsDispatchWatch *dispatch;
  BobguiPrintCupsResponseCallbackFunc ep_callback;
  BobguiCupsResult *result;

  g_assert (callback != NULL);

  ep_callback = (BobguiPrintCupsResponseCallbackFunc) callback;

  dispatch = (BobguiPrintCupsDispatchWatch *) source;

  result = bobgui_cups_request_get_result (dispatch->request);

  BOBGUI_DEBUG (PRINTING, "CUPS Backend: %s <source %p>", G_STRFUNC, source);

  if (bobgui_cups_result_is_error (result))
    {
      BOBGUI_DEBUG (PRINTING, "Error result: %s (type %i, status %i, code %i)",
                           bobgui_cups_result_get_error_string (result),
                           bobgui_cups_result_get_error_type (result),
                           bobgui_cups_result_get_error_status (result),
                           bobgui_cups_result_get_error_code (result));
     }

  ep_callback (BOBGUI_PRINT_BACKEND (dispatch->backend), result, user_data);
...
cups_dispatch_watch_finalize (GSource *source)
{
  ...
  result = bobgui_cups_request_get_result (dispatch->request);
  if (bobgui_cups_result_get_error_type (result) == BOBGUI_CUPS_ERROR_AUTH)
    {
      const char *username;
      char         hostname[HTTP_MAX_URI];
      char        *key;
      ...
      key = g_strconcat (username, "@", hostname, NULL);
      BOBGUI_DEBUG (PRINTING, "CUPS backend: removing stored password for %s", key);
      g_hash_table_remove (dispatch->backend->auth, key);
      g_free (key);
  ...
  bobgui_cups_request_free (dispatch->request);
  ...
  if (dispatch->backend)
    {
       ...
       * of print backends. See _bobgui_print_backend_create for the
       * disabling.
       */
...
static void
cups_request_execute (BobguiPrintBackendCups              *print_backend,
                      BobguiCupsRequest                   *request,
                      BobguiPrintCupsResponseCallbackFunc  callback,
                      gpointer                          user_data,
                      GDestroyNotify                    notify)
{
  ...
  dispatch = (BobguiPrintCupsDispatchWatch *) g_source_new (&_cups_dispatch_watch_funcs,
                                                         sizeof (BobguiPrintCupsDispatchWatch));
  g_source_set_static_name (&dispatch->source, "BOBGUI CUPS backend");

  BOBGUI_DEBUG (PRINTING, "CUPS Backend: %s <source %p> - Executing cups request on server '%s' and resource '%s'",
                       G_STRFUNC, dispatch, request->server, request->resource);

  dispatch->request = request;
  dispatch->backend = g_object_ref (print_backend);
  dispatch->poll_state = BOBGUI_CUPS_HTTP_IDLE;
  dispatch->data_poll = NULL;
...
typedef struct {
  BobguiPrintBackendCups *print_backend;
  BobguiPrintJob *job;
  int job_id;
  int counter;
} CupsJobPollData;
...
static void
cups_request_job_info_cb (BobguiPrintBackendCups *print_backend,
                          BobguiCupsResult       *result,
                          gpointer             user_data)
{
  CupsJobPollData *data = user_data;
  ...
  switch (state)
    ...
    case IPP_JSTATE_STOPPED:
      bobgui_print_job_set_status (data->job, BOBGUI_PRINT_STATUS_PENDING);
      break;
    case IPP_JSTATE_PROCESSING:
      bobgui_print_job_set_status (data->job, BOBGUI_PRINT_STATUS_PRINTING);
      break;
    default:
    case IPP_JSTATE_CANCELED:
    case IPP_JSTATE_ABORTED:
      bobgui_print_job_set_status (data->job, BOBGUI_PRINT_STATUS_FINISHED_ABORTED);
      done = TRUE;
      break;
    case 0:
    case IPP_JSTATE_COMPLETED:
      bobgui_print_job_set_status (data->job, BOBGUI_PRINT_STATUS_FINISHED);
      done = TRUE;
      break;
    }
  ...
  if (!done && data->job != NULL)
    {
      ...
      g_source_set_name_by_id (id, "[bobgui] cups_job_info_poll_timeout");
    }
  else
    cups_job_poll_data_free (data);
}
...
cups_request_job_info (CupsJobPollData *data)
{
  ...
  cups_request_execute (data->print_backend,
                        request,
                        (BobguiPrintCupsResponseCallbackFunc) cups_request_job_info_cb,
                        data,
                        NULL);
}
...
static void
cups_begin_polling_info (BobguiPrintBackendCups *print_backend,
			 BobguiPrintJob         *job,
			 int                  job_id)
{
      ...
      if (iter == NULL)
        {
          ...
          create_temporary_queue (BOBGUI_PRINT_BACKEND_CUPS (backend),
...
static int
find_printer (BobguiPrinter  *printer,
	      const char *find_name)
{
  const char *printer_name;

  printer_name = bobgui_printer_get_name (printer);
...
typedef enum
  {
    BOBGUI_PRINTER_STATE_LEVEL_NONE = 0,
    BOBGUI_PRINTER_STATE_LEVEL_INFO = 1,
    BOBGUI_PRINTER_STATE_LEVEL_WARNING = 2,
    BOBGUI_PRINTER_STATE_LEVEL_ERROR = 3
  } PrinterStateLevel;
...
static void
cups_printer_handle_attribute (BobguiPrintBackendCups *cups_backend,
			       ipp_attribute_t *attr,
	  ...
	  if (strcmp (ippGetString (attr, i, NULL), "none") != 0)
	      ...
	      if (interested_in)
		{
		  if (g_str_has_suffix (ippGetString (attr, i, NULL), "-report"))
		    {
		      if (info->reason_level <= BOBGUI_PRINTER_STATE_LEVEL_INFO)
			{
			  info->reason_msg = ippGetString (attr, i, NULL);
			  info->reason_level = BOBGUI_PRINTER_STATE_LEVEL_INFO;
			}
		    }
		  else if (g_str_has_suffix (ippGetString (attr, i, NULL), "-warning"))
		    {
		      if (info->reason_level <= BOBGUI_PRINTER_STATE_LEVEL_WARNING)
			{
			  info->reason_msg = ippGetString (attr, i, NULL);
			  info->reason_level = BOBGUI_PRINTER_STATE_LEVEL_WARNING;
			}
		    }
		  else  /* It is error in the case of no suffix. */
		    {
		      info->reason_msg = ippGetString (attr, i, NULL);
		      info->reason_level = BOBGUI_PRINTER_STATE_LEVEL_ERROR;
		    }
		}
	    }
	}
...
cups_create_printer (BobguiPrintBackendCups *cups_backend,
		     PrinterSetupInfo *info)
{
  BobguiPrinterCups *cups_printer;
...
#else
  cups_printer = bobgui_printer_cups_new (info->printer_name, backend, NULL);
#endif
  ...
  if (info->member_uris)
    {
      ...
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Found class with printer %s",
                           info->member_uris);
    }
  else
    {
      cups_printer->printer_uri = g_strdup (info->printer_uri);
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Found printer %s",
                           info->printer_uri);
    }
  ...
  if (strncmp (resource, "/printers/", 10) == 0)
    {
      ...
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Setting ppd name '%s' for printer/class '%s'",
  ...
  printer = BOBGUI_PRINTER (cups_printer);

  if (cups_backend->default_printer != NULL &&
      strcmp (cups_backend->default_printer, bobgui_printer_get_name (printer)) == 0)
...
static void
set_printer_icon_name_from_info (BobguiPrinter       *printer,
                                 PrinterSetupInfo *info)
{
  /* Set printer icon according to importance
     (none, report, warning, error - report is omitted). */
  if (info->reason_level == BOBGUI_PRINTER_STATE_LEVEL_ERROR)
    bobgui_printer_set_icon_name (printer, "printer-error");
  else if (info->reason_level == BOBGUI_PRINTER_STATE_LEVEL_WARNING)
    bobgui_printer_set_icon_name (printer, "printer-warning");
  else if (bobgui_printer_is_paused (printer))
    bobgui_printer_set_icon_name (printer, "printer-paused");
  else
                     ...
                     const char *printer_name)
{
  ...
  switch (i)
    {
      case 0:
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is low on toner."),
                                           printer_name);
        break;
      case 1:
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ has no toner left."),
                                           printer_name);
        break;
      case 2:
        /* Translators: "Developer" like on photo development context */
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is low on developer."),
                                           printer_name);
        break;
      case 3:
        /* Translators: "Developer" like on photo development context */
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is out of developer."),
                                           printer_name);
        break;
      case 4:
        /* Translators: "marker" is one color bin of the printer */
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is low on at least one marker supply."),
                                           printer_name);
        break;
      case 5:
        /* Translators: "marker" is one color bin of the printer */
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is out of at least one marker supply."),
                                           printer_name);
        break;
      case 6:
        reason_msg_desc = g_strdup_printf (_("The cover is open on printer â€œ%sâ€."),
                                           printer_name);
        break;
      case 7:
        reason_msg_desc = g_strdup_printf (_("The door is open on printer â€œ%sâ€."),
                                           printer_name);
        break;
      case 8:
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is low on paper."),
                                           printer_name);
        break;
      case 9:
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is out of paper."),
                                           printer_name);
        break;
      case 10:
        reason_msg_desc = g_strdup_printf (_("Printer â€œ%sâ€ is currently offline."),
                                           printer_name);
        break;
      case 11:
        reason_msg_desc = g_strdup_printf (_("There is a problem on printer â€œ%sâ€."),
                                           printer_name);
        break;
      default:
  ...
  if (info->reason_msg)
    {
      ...
      if (!found)
        info->reason_level = BOBGUI_PRINTER_STATE_LEVEL_NONE;
...
static void
set_default_printer (BobguiPrintBackendCups *cups_backend,
                     const char          *default_printer_name)
{
  ...
  if (cups_backend->default_printer != NULL)
    {
      BobguiPrinter *default_printer = NULL;
      default_printer = bobgui_print_backend_find_printer (BOBGUI_PRINT_BACKEND (cups_backend),
                                                        cups_backend->default_printer);
      if (default_printer != NULL)
        {
          bobgui_printer_set_is_default (default_printer, TRUE);
          g_signal_emit_by_name (BOBGUI_PRINT_BACKEND (cups_backend),
...
static void
cups_request_printer_info_cb (BobguiPrintBackendCups *cups_backend,
                              BobguiCupsResult       *result,
                              gpointer             user_data)
{
  ...
  if (bobgui_cups_result_is_error (result))
    {
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Error getting printer info: %s %d %d",
                           bobgui_cups_result_get_error_string (result),
                           bobgui_cups_result_get_error_type (result),
                           bobgui_cups_result_get_error_code (result));
      ...
      if (info->printer_name && info->printer_uri)
        {
          ...
          bobgui_printer_set_is_paused (printer, info->is_paused);
          bobgui_printer_set_is_accepting_jobs (printer, info->is_accepting_jobs);

          BOBGUI_PRINTER_CUPS (printer)->remote = info->remote_printer;
          ...
          BOBGUI_PRINTER_CUPS (printer)->supports_copies = info->supports_copies;
          BOBGUI_PRINTER_CUPS (printer)->supports_collate = info->supports_collate;
          BOBGUI_PRINTER_CUPS (printer)->supports_number_up = info->supports_number_up;
          BOBGUI_PRINTER_CUPS (printer)->number_of_covers = info->number_of_covers;
          BOBGUI_PRINTER_CUPS (printer)->covers = g_strdupv (info->covers);
          ...
          status_changed |= bobgui_printer_set_description (printer, info->description);
          status_changed |= bobgui_printer_set_state_message (printer, info->state_msg);
          status_changed |= bobgui_printer_set_is_accepting_jobs (printer, info->is_accepting_jobs);

          set_printer_icon_name_from_info (printer, info);

          BOBGUI_PRINTER_CUPS (printer)->media_default = info->media_default;
          BOBGUI_PRINTER_CUPS (printer)->media_supported = info->media_supported;
          BOBGUI_PRINTER_CUPS (printer)->media_size_supported = info->media_size_supported;
          if (info->media_margin_default_set)
            {
              BOBGUI_PRINTER_CUPS (printer)->media_margin_default_set = TRUE;
              BOBGUI_PRINTER_CUPS (printer)->media_bottom_margin_default = info->media_bottom_margin_default;
              BOBGUI_PRINTER_CUPS (printer)->media_top_margin_default = info->media_top_margin_default;
              BOBGUI_PRINTER_CUPS (printer)->media_left_margin_default = info->media_left_margin_default;
              BOBGUI_PRINTER_CUPS (printer)->media_right_margin_default = info->media_right_margin_default;
            }
          BOBGUI_PRINTER_CUPS (printer)->sides_default = info->sides_default;
          BOBGUI_PRINTER_CUPS (printer)->sides_supported = info->sides_supported;
          ...
          if (status_changed)
            g_signal_emit_by_name (BOBGUI_PRINT_BACKEND (backend),
...
cups_request_printer_info (BobguiPrinterCups *printer)
{
  ...
  BobguiPrintBackendCups    *backend = BOBGUI_PRINT_BACKEND_CUPS (bobgui_printer_get_backend (BOBGUI_PRINTER (printer)));
  BobguiCupsRequest         *request;
  http_t                 *http;
...
static BobguiPrinter *
find_printer_by_uuid (BobguiPrintBackendCups *backend,
                      const char          *UUID)
{
  BobguiPrinterCups *printer;
  ...
  printers = bobgui_print_backend_get_printer_list (BOBGUI_PRINT_BACKEND (backend));
 ...
 */
static void
create_temporary_queue (BobguiPrintBackendCups *backend,
                        const gchar         *printer_name,
                        const gchar         *printer_uri,
                        const gchar         *device_uri)
{
  ...
  BOBGUI_DEBUG (PRINTING, "CUPS Backend: Creating local printer %s", printer_name);
  ...
  bobgui_cups_request_ipp_add_string (request, IPP_TAG_PRINTER, IPP_TAG_NAME,
                                   "printer-name", NULL, printer_name);
  bobgui_cups_request_ipp_add_string (request, IPP_TAG_PRINTER, IPP_TAG_URI,
                                   "device-uri", NULL, device_uri);

  cups_request_execute (backend,
                        request,
                        (BobguiPrintCupsResponseCallbackFunc) cups_create_local_printer_cb,
                        NULL,
                        NULL);
}

/*
 *  Create new BobguiPrinter from information included in TXT records.
 */
static void
  ...
  if (printer == NULL)
    {
      ...
      BOBGUI_PRINTER_CUPS (printer)->printer_uri = g_strdup (data->printer_uri);
      BOBGUI_PRINTER_CUPS (printer)->temporary_queue_device_uri = g_strdup (data->device_uri);
      g_free (BOBGUI_PRINTER_CUPS (printer)->hostname);
      BOBGUI_PRINTER_CUPS (printer)->hostname = g_strdup (data->hostname);
      ...
      if (!bobgui_printer_is_active (printer))
      ...
      if (!data->backend->got_default_printer &&
          bobgui_print_backend_printer_list_is_done (BOBGUI_PRINT_BACKEND (data->backend)) &&
          data->backend->avahi_default_printer != NULL)
        set_default_printer (data->backend, data->backend->avahi_default_printer);

      /* The ref is held by BobguiPrintBackend, in add_printer() */
      g_object_unref (printer);
    }
                           ...
                           gpointer      user_data)
{
  ...
  if (output)
    {
      backend = BOBGUI_PRINT_BACKEND_CUPS (user_data);
...
avahi_service_browser_signal_handler (GDBusConnection *connection,
                                      ...
                                      gpointer         user_data)
{
  BobguiPrintBackendCups *backend = BOBGUI_PRINT_BACKEND_CUPS (user_data);
  char                *name;
  char                *type;
  char                *domain;
  guint                flags;
  ...
  else if (g_strcmp0 (signal_name, "ItemRemove") == 0)
    {
      ...
      if (g_strcmp0 (type, "_ipp._tcp") == 0 ||
          g_strcmp0 (type, "_ipps._tcp") == 0)
        {
          ...
          list = bobgui_print_backend_get_printer_list (BOBGUI_PRINT_BACKEND (backend));
          for (iter = list; iter; iter = iter->next)
            {
              ...
              if (g_strcmp0 (printer->avahi_name, name) == 0 &&
                ...
                {
                  if (g_strcmp0 (bobgui_printer_get_name (BOBGUI_PRINTER (printer)),
...
avahi_service_browser_new_cb (GObject      *source_object,
                              ...
                              gpointer      user_data)
{
  BobguiPrintBackendCups *cups_backend;
  GVariant            *output;
  GError              *error = NULL;
  int                  i;
  ...
  if (output)
    {
      cups_backend = BOBGUI_PRINT_BACKEND_CUPS (user_data);
...
static void
avahi_request_printer_list (BobguiPrintBackendCups *cups_backend)
{
                              ...
                              gpointer             user_data)
{
  ...
  if (bobgui_cups_result_is_error (result))
    {
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Error getting printer list: %s %d %d",
                           bobgui_cups_result_get_error_string (result),
                           bobgui_cups_result_get_error_type (result),
                           bobgui_cups_result_get_error_code (result));

      if (bobgui_cups_result_get_error_type (result) == BOBGUI_CUPS_ERROR_AUTH &&
          bobgui_cups_result_get_error_code (result) == 1)
        {
  ...
  for (attr = ippGetFirstAttribute (response); attr != NULL;
       attr = ippGetNextAttribute (response))
    {
      ...
      printer = bobgui_print_backend_find_printer (backend, info->printer_name);
      if (!printer)
	{
	...
	g_object_ref (printer);
      ...
      if (!bobgui_printer_is_active (printer))
        {
	  bobgui_printer_set_is_active (printer, TRUE);
	  bobgui_printer_set_is_new (printer, TRUE);
          list_has_changed = TRUE;
        }

      if (bobgui_printer_is_new (printer))
        {
      ...
      BOBGUI_PRINTER_CUPS (printer)->supports_number_up = info->supports_number_up;
      BOBGUI_PRINTER_CUPS (printer)->number_of_covers = info->number_of_covers;
      g_clear_pointer (&(BOBGUI_PRINTER_CUPS (printer)->covers), g_strfreev);
      BOBGUI_PRINTER_CUPS (printer)->covers = g_strdupv (info->covers);
      BOBGUI_PRINTER_CUPS (printer)->is_temporary = info->is_temporary;
      status_changed = bobgui_printer_set_job_count (printer, info->job_count);
      status_changed |= bobgui_printer_set_location (printer, info->location);
          ...
          for (iter = removed_printer_checklist; iter; iter = iter->next)
            {
              if (!BOBGUI_PRINTER_CUPS (iter->data)->avahi_browsed)
                {
                  mark_printer_inactive (BOBGUI_PRINTER (iter->data), backend);
                  list_has_changed = TRUE;
                }
            }
        }
...
static void
update_backend_status (BobguiPrintBackendCups    *cups_backend,
                       BobguiCupsConnectionState  state)
{
  switch (state)
    {
    case BOBGUI_CUPS_CONNECTION_NOT_AVAILABLE:
      g_object_set (cups_backend, "status", BOBGUI_PRINT_BACKEND_STATUS_UNAVAILABLE, NULL);
      break;
    case BOBGUI_CUPS_CONNECTION_AVAILABLE:
      g_object_set (cups_backend, "status", BOBGUI_PRINT_BACKEND_STATUS_OK, NULL);
      break;

    case BOBGUI_CUPS_CONNECTION_IN_PROGRESS:
    default: ;
    }
}

static gboolean
cups_request_printer_list (BobguiPrintBackendCups *cups_backend)
{
  BobguiCupsConnectionState state;
  BobguiCupsRequest *request;
  ...
  state = bobgui_cups_connection_test_get_state (cups_backend->cups_connection_test);
  ...
  if (cups_backend->list_printers_attempts == 60)
    {
      ...
      g_source_set_name_by_id (cups_backend->list_printers_poll, "[bobgui] cups_request_printer_list");
    }
  else if (cups_backend->list_printers_attempts != -1)
    cups_backend->list_printers_attempts++;

  if (state == BOBGUI_CUPS_CONNECTION_IN_PROGRESS || state == BOBGUI_CUPS_CONNECTION_NOT_AVAILABLE)
    return TRUE;
  else
  ...
  cups_request_execute (cups_backend,
                        request,
                        (BobguiPrintCupsResponseCallbackFunc) cups_request_printer_list_cb,
		        request,
		        NULL);
...
static void
cups_get_printer_list (BobguiPrintBackend *backend)
{
  BobguiPrintBackendCups *cups_backend;

  cups_backend = BOBGUI_PRINT_BACKEND_CUPS (backend);

  if (cups_backend->cups_connection_test == NULL)
    cups_backend->cups_connection_test = bobgui_cups_connection_test_new (NULL, -1);
...
static void
cups_request_ppd_cb (BobguiPrintBackendCups *print_backend,
                     BobguiCupsResult       *result,
                     GetPPDData          *data)
{
  BobguiPrinter *printer;
  ...
  printer = BOBGUI_PRINTER (data->printer);
  ...
  if (data_info.st_size == 0 ||
      BOBGUI_PRINTER_CUPS (printer)->avahi_browsed ||
      (bobgui_cups_result_is_error (result) &&
         ...
         (bobgui_cups_result_get_error_status (result) == HTTP_NOT_FOUND))))
    {
      ...
      if (!cups_printer->avahi_browsed &&
           ...
           g_str_has_prefix (cups_printer->original_device_uri, "ipps://")))
        {
          ...
          bobgui_cups_connection_test_free (cups_printer->remote_cups_connection_test);
          ...
          cups_printer->remote_cups_connection_test =
            bobgui_cups_connection_test_new (cups_printer->original_hostname,
          ...
          if (cups_request_ppd (printer))
            {
              ...
              g_source_set_name_by_id (cups_printer->get_remote_ppd_poll, "[bobgui] cups_request_ppd");
            }
        }
      else
        {
...
static gboolean
cups_request_ppd (BobguiPrinter *printer)
{
  GError *error;
  BobguiPrintBackend *print_backend;
  BobguiPrinterCups *cups_printer;
  BobguiCupsRequest *request;
  ...
  if (cups_printer->remote && !cups_printer->avahi_browsed)
    {
      ...
      state = bobgui_cups_connection_test_get_state (cups_printer->remote_cups_connection_test);

      if (state == BOBGUI_CUPS_CONNECTION_IN_PROGRESS)
        {
      ...
      if (state == BOBGUI_CUPS_CONNECTION_NOT_AVAILABLE)
        {
  ...
  fd = g_file_open_tmp ("bobguiprint_ppd_XXXXXX",
                        &ppd_filename,
                        &error);
  ...
  if (error != NULL)
    {
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Failed to create temp file, %s",
                           error->message);
      g_error_free (error);
      httpClose (http);
      g_free (ppd_filename);
  ...
  BOBGUI_DEBUG (PRINTING, "CUPS Backend: Requesting resource %s to be written to temp file %s",
                       resource, ppd_filename);

  cups_printer->reading_ppd = TRUE;
  BOBGUI_PRINT_BACKEND_CUPS (print_backend)->reading_ppds++;

  cups_request_execute (BOBGUI_PRINT_BACKEND_CUPS (print_backend),
                        request,
                        (BobguiPrintCupsResponseCallbackFunc) cups_request_ppd_cb,
                        data,
 ...
 */
static void
cups_get_default_printer (BobguiPrintBackendCups *backend)
{
  BobguiPrintBackendCups *cups_backend;
      ...
      if (cups_request_default_printer (cups_backend))
        {
          ...
          g_source_set_name_by_id (cups_backend->default_printer_poll, "[bobgui] cups_request_default_printer");
        }
    }
}
...
static void
cups_get_local_default_printer (BobguiPrintBackendCups *backend)
{
  const char *str;
  char *name = NULL;
...
static void
cups_request_default_printer_cb (BobguiPrintBackendCups *print_backend,
				 BobguiCupsResult       *result,
				 gpointer             user_data)
{
  ipp_t *response;
  ...
  if (print_backend->default_printer != NULL)
    {
      ...
      if (printer != NULL)
        {
          bobgui_printer_set_is_default (printer, TRUE);
          g_signal_emit_by_name (BOBGUI_PRINT_BACKEND (print_backend), "printer-status-changed", printer);
        }
    }
...
static gboolean
cups_request_default_printer (BobguiPrintBackendCups *print_backend)
{
  BobguiCupsConnectionState state;
  BobguiCupsRequest *request;

  state = bobgui_cups_connection_test_get_state (print_backend->cups_connection_test);
  ...
  cups_request_execute (print_backend,
                        request,
                        (BobguiPrintCupsResponseCallbackFunc) cups_request_default_printer_cb,
...
static void
cups_printer_request_details (BobguiPrinter *printer)
{
  BobguiPrinterCups *cups_printer;
  ...
  if (cups_printer->avahi_browsed)
    {
      create_temporary_queue (BOBGUI_PRINT_BACKEND_CUPS (bobgui_printer_get_backend (printer)),
                              bobgui_printer_get_name (printer),
                              cups_printer->printer_uri,
                              cups_printer->temporary_queue_device_uri);
    }
      ...
      if (cups_printer->remote && !cups_printer->avahi_browsed)
        {
          if (cups_printer->get_remote_ppd_poll == 0)
            {
              cups_printer->remote_cups_connection_test =
                bobgui_cups_connection_test_new (cups_printer->hostname,
                                              cups_printer->port);
		  ...
		  const char *text)
{
  ...
  if (res == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "CUPS Backend: Unable to convert PPD text");
      res = g_strdup ("???");
    }
...
static const struct {
  ...
  { "Duplex", "bobgui-duplex" },
  { "MediaType", "bobgui-paper-type" },
  { "InputSlot", "bobgui-paper-source" },
  { "OutputBin", "bobgui-output-tray" }
};

static const struct {
  const char *ipp_option_name;
  const char *bobgui_option_name;
  const char *translation;
} ipp_option_translations[] = {
  { "sides", "bobgui-duplex", NC_("printing option", "Two Sided") },
  { "output-bin", "bobgui-output-tray", NC_("printing option", "Output Tray") }
};
...
static const struct {
  ...
  { "number-up", "bobgui-n-up" },
  { "number-up-layout", "bobgui-n-up-layout" },
  { "job-billing", "bobgui-billing-info" },
  { "job-priority", "bobgui-job-prio" }
};
...
static void
set_option_off (BobguiPrinterOption *option)
{
  /* Any of these will do, _set only applies the value
   * if its allowed of the option */
  bobgui_printer_option_set (option, "False");
  bobgui_printer_option_set (option, "Off");
  bobgui_printer_option_set (option, "None");
}
...
static BobguiPrinterOption *
create_pickone_option (ppd_file_t   *ppd_file,
		       ppd_option_t *ppd_option,
		       const char   *bobgui_name)
{
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  ...
  n_choices = available_choices (ppd_file, ppd_option, &available, g_str_has_prefix (bobgui_name, "bobgui-"));
  if (n_choices > 0)
    {
	  ...
	  ppd_cparam_t *cparam;
          ...
          if (ppdNextCustomParam (coption) == NULL)
	    {
	        ...
	        {
                case PPD_CUSTOM_INT:
		  option = bobgui_printer_option_new (bobgui_name, label,
      ...
      bobgui_printer_option_allocate_choices (option, n_choices);
      for (i = 0; i < n_choices; i++)
	{
	  if (available[i] == NULL)
	    {
	      /* This was auto-added */
	      option->choices[i] = g_strdup ("bobgui-ignore-value");
      ...
      if (option->type != BOBGUI_PRINTER_OPTION_TYPE_PICKONE)
        {
          if (g_str_has_prefix (ppd_option->defchoice, "Custom."))
            bobgui_printer_option_set (option, ppd_option->defchoice + 7);
          else
            bobgui_printer_option_set (option, ppd_option->defchoice);
        }
      else
        {
          bobgui_printer_option_set (option, ppd_option->defchoice);
  ...
  if (n_choices == 2)
    {
      ...
      bobgui_printer_option_allocate_choices (option, 2);
...
#if CUPS_VERSION_MAJOR < 3
static void
handle_option (BobguiPrinterOptionSet *set,
	       ppd_file_t          *ppd_file,
	       ppd_option_t        *ppd_option,
...
static void
handle_group (BobguiPrinterOptionSet *set,
	      ppd_file_t          *ppd_file,
	      ppd_group_t         *group,
	      ppd_group_t         *toplevel_group,
	      BobguiPrintSettings    *settings)
...
typedef struct {
        BobguiPrintSettings     *settings;
        BobguiPrinter           *printer;
} BobguiPrintBackendCupsColordHelper;

static void
colord_printer_option_set_changed_cb (BobguiPrinterOptionSet *set,
                                      BobguiPrintBackendCupsColordHelper *helper)
{
  bobgui_printer_cups_update_settings (BOBGUI_PRINTER_CUPS (helper->printer),
                                    helper->settings,
                                    set);
}
#endif
...
/*
 * Lookup translation and BOBGUI name of given IPP option name.
 */
static gboolean
get_ipp_option_translation (const char   *ipp_option_name,
                            char        **bobgui_option_name,
                            char        **translation)
{
  int i;

  *bobgui_option_name = NULL;
  *translation = NULL;
  ...
  for (i = 0; i < G_N_ELEMENTS (ipp_option_translations); i++)
    {
      if (g_strcmp0 (ipp_option_translations[i].ipp_option_name, ipp_option_name) == 0)
        {
          *bobgui_option_name = g_strdup (ipp_option_translations[i].bobgui_option_name);
...
{
  BobguiPrinterOption *option = NULL;
  char             *bobgui_option_name = NULL;
  char             *translation = NULL;
  char             *ipp_choice;
  gsize             i;
  ...
  if (bobgui_option_name != NULL)
    option = bobgui_printer_option_set_lookup (set, bobgui_option_name);
  ...
  if (option == NULL &&
      ipp_choices != NULL)
    {
      ...
      option = bobgui_printer_option_new (bobgui_option_name,
                                       translation,
                                       BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
      ...
      if (choices != NULL &&
        ...
        {
          bobgui_printer_option_choices_from_array (option,
                                                 length,
                                                 (const char **)choices,
  ...
  if (option != NULL &&
    ...
    {
      bobgui_printer_option_set (option, ipp_choice_default);
    }
...
cups_printer_get_options (BobguiPrinter           *printer,
			  BobguiPrintSettings     *settings,
			  BobguiPageSetup         *page_setup,
			  BobguiPrintCapabilities  capabilities)
{
  ...
  BobguiPrinterCups *cups_printer = NULL;
#ifdef HAVE_COLORD
  BobguiPrintBackendCupsColordHelper *helper;
#endif
   ...
   * in the print dialog
   */
  option = bobgui_printer_option_new ("bobgui-job-prio", _("Job Priority"), BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
  bobgui_printer_option_choices_from_array (option, G_N_ELEMENTS (prio),
					 prio, prio_display);
  bobgui_printer_option_set (option, "50");
  set_option_from_settings (option, settings);
  bobgui_printer_option_set_add (set, option);
   ...
   * in the print dialog
   */
  option = bobgui_printer_option_new ("bobgui-billing-info", _("Billing Info"), BOBGUI_PRINTER_OPTION_TYPE_STRING);
  bobgui_printer_option_set (option, "");
  set_option_from_settings (option, settings);
  bobgui_printer_option_set_add (set, option);
  g_object_unref (option);

  backend = BOBGUI_PRINT_BACKEND_CUPS (bobgui_printer_get_backend (printer));
  cups_printer = BOBGUI_PRINTER_CUPS (printer);
  ...
  if (backend != NULL && printer != NULL)
    {
      ...
      int j;
        ...
        * in the print dialog
        */
      option = bobgui_printer_option_new ("bobgui-n-up", C_("printer option", "Pages per Sheet"), BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
      bobgui_printer_option_choices_from_array (option, G_N_ELEMENTS (n_up), n_up, n_up);
      default_number_up = g_strdup_printf ("%d", cups_printer->default_number_up);
      bobgui_printer_option_set (option, default_number_up);
      g_free (default_number_up);
      set_option_from_settings (option, settings);
      bobgui_printer_option_set_add (set, option);
      g_object_unref (option);

      if (cups_printer_get_capabilities (printer) & BOBGUI_PRINT_CAPABILITY_NUMBER_UP_LAYOUT)
        {
          for (i = 0; i < G_N_ELEMENTS (n_up_layout_display); i++)
            ...
            */
          option = bobgui_printer_option_new ("bobgui-n-up-layout", C_("printer option", "Page Ordering"), BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
          bobgui_printer_option_choices_from_array (option, G_N_ELEMENTS (n_up_layout),
                                                 n_up_layout, n_up_layout_display);

          text_direction = bobgui_widget_get_default_direction ();
          if (text_direction == BOBGUI_TEXT_DIR_LTR)
            bobgui_printer_option_set (option, "lrtb");
          else
            bobgui_printer_option_set (option, "rltb");
       ...
       */
      option = bobgui_printer_option_new ("bobgui-cover-before", C_("printer option", "Before"), BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
      bobgui_printer_option_choices_from_array (option, num_of_covers,
                                             (const char **)cover, (const char **)cover_display_translated);

      if (cups_printer->default_cover_before != NULL)
        bobgui_printer_option_set (option, cups_printer->default_cover_before);
      else
        bobgui_printer_option_set (option, "none");
      set_option_from_settings (option, settings);
      bobgui_printer_option_set_add (set, option);
       ...
       */
      option = bobgui_printer_option_new ("bobgui-cover-after", C_("printer option", "After"), BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
      bobgui_printer_option_choices_from_array (option, num_of_covers,
                                             (const char **)cover, (const char **)cover_display_translated);
      if (cups_printer->default_cover_after != NULL)
        bobgui_printer_option_set (option, cups_printer->default_cover_after);
      else
        bobgui_printer_option_set (option, "none");
      set_option_from_settings (option, settings);
      bobgui_printer_option_set_add (set, option);
   ...
   * or 'on hold'
   */
  option = bobgui_printer_option_new ("bobgui-print-time", C_("printer option", "Print at"), BOBGUI_PRINTER_OPTION_TYPE_PICKONE);
  bobgui_printer_option_choices_from_array (option, G_N_ELEMENTS (print_at),
					 print_at, print_at);
   ...
   */
  option = bobgui_printer_option_new ("bobgui-print-time-text", C_("printer option", "Print at time"), BOBGUI_PRINTER_OPTION_TYPE_STRING);
  bobgui_printer_option_set (option, "");
  set_option_from_settings (option, settings);
  bobgui_printer_option_set_add (set, option);
  g_object_unref (option);
  ...
  if (ppd_file)
    {
      ...
      if (ppd_option)
	{
	  ppd_name = bobgui_paper_size_get_ppd_name (paper_size);
	  ...
	  if (ppd_name)
	  ...
	  else
	    {
	       ...
	       * 230.4x142.9"
               */
	      custom_name = g_strdup_printf (_("Custom %sÃ—%s"), width, height);
  ...
  num_opts = cups_get_user_options (bobgui_printer_get_name (printer), 0, &opts);
      ...
      if (strcmp (name, "cups-job-sheets") == 0)
        {
          ...
          option = bobgui_printer_option_set_lookup (set, "bobgui-cover-before");
          if (option && num_values > 0)
            bobgui_printer_option_set (option, g_strstrip (values[0]));

          option = bobgui_printer_option_set_lookup (set, "bobgui-cover-after");
          if (option && num_values > 1)
            bobgui_printer_option_set (option, g_strstrip (values[1]));

          g_strfreev (values);
        }
      else if (strcmp (name, "cups-job-hold-until") == 0)
        {
          BobguiPrinterOption *option2 = NULL;

          option = bobgui_printer_option_set_lookup (set, "bobgui-print-time-text");
          if (option && opts[i].value)
            {
              option2 = bobgui_printer_option_set_lookup (set, "bobgui-print-time");
              if (option2)
                {
                  if (strcmp (opts[i].value, "indefinite") == 0)
                    bobgui_printer_option_set (option2, "on-hold");
                  else
                    {
                      bobgui_printer_option_set (option2, "at");
                      bobgui_printer_option_set (option, opts[i].value);
                    }
                }
            }
        }
        ...
        {
          option = bobgui_printer_option_set_lookup (set, "bobgui-duplex");
          if (option && opts[i].value)
            {
              if (!option_is_ipp_option (option))
                {
                  if (strcmp (opts[i].value, "two-sided-short-edge") == 0)
                    bobgui_printer_option_set (option, "DuplexTumble");
                  else if (strcmp (opts[i].value, "two-sided-long-edge") == 0)
                    bobgui_printer_option_set (option, "DuplexNoTumble");
                }
              else
                {
                  bobgui_printer_option_set (option, opts[i].value);
        ...
        {
          option = bobgui_printer_option_set_lookup (set, name);
          if (option)
            bobgui_printer_option_set (option, opts[i].value);
        }
      g_free (name);
...
#ifdef HAVE_COLORD
  option = bobgui_printer_option_new ("colord-profile",
  ...
  bobgui_printer_option_set (option, C_("printer option value", "Unavailable"));
  bobgui_printer_option_set_add (set, option);

  /* watch to see if the user changed the options */
  helper = g_new (BobguiPrintBackendCupsColordHelper, 1);
  ...
  /* initial coldplug */
  bobgui_printer_cups_update_settings (BOBGUI_PRINTER_CUPS (printer),
                                    settings, set);
...
#if CUPS_VERSION_MAJOR < 3
static void
mark_option_from_set (BobguiPrinterOptionSet *set,
		      ppd_file_t          *ppd_file,
		      ppd_option_t        *ppd_option)
{
  BobguiPrinterOption *option;
...
static void
mark_group_from_set (BobguiPrinterOptionSet *set,
		     ppd_file_t          *ppd_file,
		     ppd_group_t         *group)
{
  int i;
...
static void
set_conflicts_from_option (BobguiPrinterOptionSet *set,
			   ppd_file_t          *ppd_file,
			   ppd_option_t        *ppd_option)
{
  BobguiPrinterOption *option;
  ...
  if (ppd_option->conflicted)
    {
      ...
      if (option)
	bobgui_printer_option_set_has_conflict (option, TRUE);
#ifdef PRINT_IGNORED_OPTIONS
      else
...
#if CUPS_VERSION_MAJOR < 3
static void
set_conflicts_from_group (BobguiPrinterOptionSet *set,
			  ppd_file_t          *ppd_file,
			  ppd_group_t         *group)
{
  int i;
...
static gboolean
cups_printer_mark_conflicts (BobguiPrinter          *printer,
			     BobguiPrinterOptionSet *options)
{
#if CUPS_VERSION_MAJOR < 3
  ppd_file_t *ppd_file;
...
static void
map_settings_to_option (BobguiPrinterOption  *option,
			...
			const char        *ipp_name)
{
  ...
  cups_value = bobgui_print_settings_get (settings, name);
  g_free (name);

  if (cups_value != NULL)
    {
      bobgui_printer_option_set (option, cups_value);
      return;
    }

  /* If the IPP-specific setting is set, use that */
  name = g_strdup_printf ("cups-%s", ipp_name);
  ipp_value = bobgui_print_settings_get (settings, name);
  g_free (name);

  if (ipp_value != NULL)
    {
      bobgui_printer_option_set (option, ipp_value);
      return;
    }

  /* Otherwise we try to convert from the general setting */
  standard_value = bobgui_print_settings_get (settings, standard_name);
  ...
  for (i = 0; i < n_elements; i++)
	...
	{
	  bobgui_printer_option_set (option, standard_value);
	  break;
	}
	...
	{
	  bobgui_printer_option_set (option, table[i].cups);
	  break;
	}
    }
}
			...
			gboolean           is_ipp_option)
{
  ...
  bobgui_print_settings_set (settings, name, value);
...
static void
set_option_from_settings (BobguiPrinterOption *option,
			  BobguiPrintSettings *settings)
{
  const char *cups_value;
  char *value;
  ...
  if (strcmp (option->name, "bobgui-paper-source") == 0)
    map_settings_to_option (option, paper_source_map, G_N_ELEMENTS (paper_source_map),
			     settings, BOBGUI_PRINT_SETTINGS_DEFAULT_SOURCE,
			     "InputSlot", NULL);
  else if (strcmp (option->name, "bobgui-output-tray") == 0)
    map_settings_to_option (option, output_tray_map, G_N_ELEMENTS (output_tray_map),
			    settings, BOBGUI_PRINT_SETTINGS_OUTPUT_BIN,
			    "OutputBin", "output-bin");
  else if (strcmp (option->name, "bobgui-duplex") == 0)
    map_settings_to_option (option, duplex_map, G_N_ELEMENTS (duplex_map),
			    settings, BOBGUI_PRINT_SETTINGS_DUPLEX,
			    "Duplex", "sides");
  else if (strcmp (option->name, "cups-OutputMode") == 0)
    map_settings_to_option (option, output_mode_map, G_N_ELEMENTS (output_mode_map),
			    settings, BOBGUI_PRINT_SETTINGS_QUALITY,
			    "OutputMode", NULL);
  else if (strcmp (option->name, "cups-Resolution") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, option->name);
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
      else
	{
	  if (bobgui_print_settings_get_int_with_default (settings, BOBGUI_PRINT_SETTINGS_RESOLUTION, -1) != -1 ||
	      bobgui_print_settings_get_int_with_default (settings, BOBGUI_PRINT_SETTINGS_RESOLUTION_X, -1) != -1 ||
	      bobgui_print_settings_get_int_with_default (settings, BOBGUI_PRINT_SETTINGS_RESOLUTION_Y, -1) != -1 ||
	      option->value == NULL || option->value[0] == '\0')
	    {
              int res = bobgui_print_settings_get_resolution (settings);
              int res_x = bobgui_print_settings_get_resolution_x (settings);
              int res_y = bobgui_print_settings_get_resolution_y (settings);

              if (res_x != res_y)
                {
                  value = g_strdup_printf ("%dx%ddpi", res_x, res_y);
                  bobgui_printer_option_set (option, value);
                  g_free (value);
                }
              else if (res != 0)
                {
  ...
  else if (strcmp (option->name, "bobgui-paper-type") == 0)
    map_settings_to_option (option, media_type_map, G_N_ELEMENTS (media_type_map),
			    settings, BOBGUI_PRINT_SETTINGS_MEDIA_TYPE,
			    "MediaType", NULL);
  else if (strcmp (option->name, "bobgui-n-up") == 0)
    {
      map_settings_to_option (option, all_map, G_N_ELEMENTS (all_map),
			      settings, BOBGUI_PRINT_SETTINGS_NUMBER_UP,
			      "number-up", NULL);
    }
  else if (strcmp (option->name, "bobgui-n-up-layout") == 0)
    {
      map_settings_to_option (option, all_map, G_N_ELEMENTS (all_map),
			      settings, BOBGUI_PRINT_SETTINGS_NUMBER_UP_LAYOUT,
			      "number-up-layout", NULL);
    }
  else if (strcmp (option->name, "bobgui-billing-info") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, "cups-job-billing");
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
    }
  else if (strcmp (option->name, "bobgui-job-prio") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, "cups-job-priority");
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
    }
  else if (strcmp (option->name, "bobgui-cover-before") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, "cover-before");
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
    }
  else if (strcmp (option->name, "bobgui-cover-after") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, "cover-after");
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
    }
  else if (strcmp (option->name, "bobgui-print-time") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, "print-at");
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
    }
  else if (strcmp (option->name, "bobgui-print-time-text") == 0)
    {
      cups_value = bobgui_print_settings_get (settings, "print-at-time");
      if (cups_value)
	bobgui_printer_option_set (option, cups_value);
    }
...
static void
foreach_option_get_settings (BobguiPrinterOption *option,
			     gpointer          user_data)
{
  struct OptionData *data = user_data;
  BobguiPrintSettings *settings = data->settings;
  const char *value;
  ...
  if (strcmp (option->name, "bobgui-paper-source") == 0)
  ...
  else if (strcmp (option->name, "cups-Resolution") == 0)
    {
      ...
      bobgui_print_settings_set (settings, option->name, value);
    }
  else if (strcmp (option->name, "bobgui-paper-type") == 0)
  ...
  else if (strcmp (option->name, "bobgui-billing-info") == 0 && strlen (value) > 0)
    bobgui_print_settings_set (settings, "cups-job-billing", value);
  else if (strcmp (option->name, "bobgui-job-prio") == 0)
    bobgui_print_settings_set (settings, "cups-job-priority", value);
  else if (strcmp (option->name, "bobgui-cover-before") == 0)
    bobgui_print_settings_set (settings, "cover-before", value);
  else if (strcmp (option->name, "bobgui-cover-after") == 0)
    bobgui_print_settings_set (settings, "cover-after", value);
  else if (strcmp (option->name, "bobgui-print-time") == 0)
    bobgui_print_settings_set (settings, "print-at", value);
  else if (strcmp (option->name, "bobgui-print-time-text") == 0)
    bobgui_print_settings_set (settings, "print-at-time", value);
  else if (g_str_has_prefix (option->name, "cups-"))
    bobgui_print_settings_set (settings, option->name, value);
}

static void
cups_printer_get_settings_from_options (BobguiPrinter          *printer,
					BobguiPrinterOptionSet *options,
					BobguiPrintSettings    *settings)
{
...
#if CUPS_VERSION_MAJOR < 3
  data.ppd_file = bobgui_printer_cups_get_ppd (BOBGUI_PRINTER_CUPS (printer));
#endif

  bobgui_printer_option_set_foreach (options, foreach_option_get_settings, &data);
#if CUPS_VERSION_MAJOR < 3
  if (data.ppd_file != NULL)
    {
      BobguiPrinterOption *cover_before, *cover_after;
      ...
      if (cover_before && cover_after)
	{
	  ...
	  bobgui_print_settings_set (settings, "cups-job-sheets", value);
	  g_free (value);
	}

      print_at = bobgui_print_settings_get (settings, "print-at");
      print_at_time = bobgui_print_settings_get (settings, "print-at-time");
      ...
      if (strcmp (print_at, "at") == 0)
        {
          ...
          if (utc_time != NULL)
            {
              bobgui_print_settings_set (settings, "cups-job-hold-until", utc_time);
              g_free (utc_time);
            }
          else
            bobgui_print_settings_set (settings, "cups-job-hold-until", print_at_time);
        }
      else if (strcmp (print_at, "on-hold") == 0)
	bobgui_print_settings_set (settings, "cups-job-hold-until", "indefinite");
    }
#endif
}

static void
cups_printer_prepare_for_print (BobguiPrinter       *printer,
				BobguiPrintJob      *print_job,
				BobguiPrintSettings *settings,
				BobguiPageSetup     *page_setup)
{
  ...
  if (capabilities & BOBGUI_PRINT_CAPABILITY_COLLATE)
    {
      if (bobgui_print_settings_get_collate (settings))
        bobgui_print_settings_set (settings, "cups-Collate", "True");
      else
        bobgui_print_settings_set (settings, "cups-Collate", "False");
  ...
  if (capabilities & BOBGUI_PRINT_CAPABILITY_REVERSE)
    {
      if (bobgui_print_settings_get_reverse (settings))
        bobgui_print_settings_set (settings, "cups-OutputOrder", "Reverse");
  ...
  if (capabilities & BOBGUI_PRINT_CAPABILITY_COPIES)
    {
      if (bobgui_print_settings_get_n_copies (settings) > 1)
        bobgui_print_settings_set_int (settings, "cups-copies",
                                    bobgui_print_settings_get_n_copies (settings));
  ...
  if (page_set == BOBGUI_PAGE_SET_EVEN)
    bobgui_print_settings_set (settings, "cups-page-set", "even");
  else if (page_set == BOBGUI_PAGE_SET_ODD)
    bobgui_print_settings_set (settings, "cups-page-set", "odd");
  bobgui_print_job_set_page_set (print_job, BOBGUI_PAGE_SET_ALL);

  paper_size = bobgui_page_setup_get_paper_size (page_setup);
  ppd_paper_name = bobgui_paper_size_get_ppd_name (paper_size);
  if (ppd_paper_name != NULL)
    bobgui_print_settings_set (settings, "cups-PageSize", ppd_paper_name);
  else if (bobgui_paper_size_is_ipp (paper_size))
    bobgui_print_settings_set (settings, "cups-media", bobgui_paper_size_get_name (paper_size));
  else
    {
      char width[G_ASCII_DTOSTR_BUF_SIZE];
      char height[G_ASCII_DTOSTR_BUF_SIZE];
      ...
      g_ascii_formatd (width, sizeof (width), "%.2f", bobgui_paper_size_get_width (paper_size, BOBGUI_UNIT_POINTS));
      g_ascii_formatd (height, sizeof (height), "%.2f", bobgui_paper_size_get_height (paper_size, BOBGUI_UNIT_POINTS));
      custom_name = g_strdup_printf (("Custom.%sx%s"), width, height);
      bobgui_print_settings_set (settings, "cups-PageSize", custom_name);
      g_free (custom_name);
    }

  if (bobgui_print_settings_get_number_up (settings) > 1)
    {
      BobguiNumberUpLayout  layout = bobgui_print_settings_get_number_up_layout (settings);
      ...
      bobgui_print_settings_set (settings, "cups-number-up-layout", enum_value->value_nick);
      g_type_class_unref (enum_class);

      if (!(capabilities & BOBGUI_PRINT_CAPABILITY_NUMBER_UP))
        {
		   ...
		   ppd_size_t *size)
{
  ...
  paper_size = bobgui_paper_size_new_from_ppd (size->name,
					    display_name,
					    size->width,
					    size->length);
  ...
  bobgui_page_setup_set_top_margin (page_setup, size->length - size->top, BOBGUI_UNIT_POINTS);
  bobgui_page_setup_set_bottom_margin (page_setup, size->bottom, BOBGUI_UNIT_POINTS);
  bobgui_page_setup_set_left_margin (page_setup, size->left, BOBGUI_UNIT_POINTS);
  bobgui_page_setup_set_right_margin (page_setup, size->width - size->right, BOBGUI_UNIT_POINTS);
                              ...
                              int        media_right_margin_default)
{
  ...
  page_setup = bobgui_page_setup_new ();
  paper_size = bobgui_paper_size_new_from_ipp (media,
  ...
  if (media_margin_default_set)
    {
      bobgui_page_setup_set_bottom_margin (page_setup, media_bottom_margin_default, BOBGUI_UNIT_MM);
      bobgui_page_setup_set_top_margin (page_setup, media_top_margin_default, BOBGUI_UNIT_MM);
      bobgui_page_setup_set_left_margin (page_setup, media_left_margin_default, BOBGUI_UNIT_MM);
      bobgui_page_setup_set_right_margin (page_setup, media_right_margin_default, BOBGUI_UNIT_MM);
    }
...
static GList *
cups_printer_list_papers (BobguiPrinter *printer)
{
  BobguiPageSetup *page_setup;
  BobguiPrinterCups *cups_printer = BOBGUI_PRINTER_CUPS (printer);
  GList *result = NULL;
  ...
  ppd_file = bobgui_printer_cups_get_ppd (cups_printer);
  if (ppd_file != NULL)
    {
...
static BobguiPageSetup *
cups_printer_get_default_page_size (BobguiPrinter *printer)
{
  BobguiPrinterCups *cups_printer = BOBGUI_PRINTER_CUPS (printer);
...
static gboolean
cups_printer_get_hard_margins (BobguiPrinter *printer,
			       double     *top,
			       double     *bottom,
			       double     *left,
			       double     *right)
					      ...
					      double       *right)
{
  ...
  paper_name = bobgui_paper_size_get_ppd_name (paper_size);
...
cups_printer_get_capabilities (BobguiPrinter *printer)
{
  BobguiPrintCapabilities  capabilities = 0;
  BobguiPrinterCups       *cups_printer = BOBGUI_PRINTER_CUPS (printer);

#if CUPS_VERSION_MAJOR < 3
  if (bobgui_printer_cups_get_ppd (cups_printer))
    {

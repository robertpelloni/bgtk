...
static void bobgui_printer_cups_finalize   (GObject             *object);

static void bobgui_printer_cups_set_property (GObject      *object,
                                           guint         prop_id,
                                           const GValue *value,
                                           GParamSpec   *pspec);
static void bobgui_printer_cups_get_property (GObject      *object,
                                           guint         prop_id,
                                           GValue       *value,
                                           GParamSpec   *pspec);

G_DEFINE_TYPE (BobguiPrinterCups, bobgui_printer_cups, BOBGUI_TYPE_PRINTER);

static void
bobgui_printer_cups_class_init (BobguiPrinterCupsClass *class)
{
  GObjectClass *object_class = (GObjectClass *) class;

  object_class->finalize = bobgui_printer_cups_finalize;
  object_class->set_property = bobgui_printer_cups_set_property;
  object_class->get_property = bobgui_printer_cups_get_property;

  bobgui_printer_cups_parent_class = g_type_class_peek_parent (class);
...
static void
bobgui_printer_cups_init (BobguiPrinterCups *printer)
{
...
static void
bobgui_printer_cups_finalize (GObject *object)
{
  BobguiPrinterCups *printer;

  g_return_if_fail (object != NULL);

  printer = BOBGUI_PRINTER_CUPS (object);
  ...
  bobgui_cups_connection_test_free (printer->remote_cups_connection_test);

  G_OBJECT_CLASS (bobgui_printer_cups_parent_class)->finalize (object);
}

static void
bobgui_printer_cups_set_property (GObject         *object,
                               guint            prop_id,
                               const GValue    *value,
                               GParamSpec      *pspec)
{
...
static void
bobgui_printer_cups_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
#ifdef HAVE_COLORD
  BobguiPrinterCups *printer = BOBGUI_PRINTER_CUPS (object);
#endif
...
static void
colord_update_ui_from_settings (BobguiPrinterCups *printer)
{
...
void
bobgui_printer_cups_update_settings (BobguiPrinterCups *printer,
                                  ...
                                  BobguiPrinterOptionSet *set)
{
  ...
  /* cupsICCQualifier1 */
  option = bobgui_printer_option_set_lookup (set, "cups-ColorSpace");
  if (option == NULL)
    option = bobgui_printer_option_set_lookup (set, "cups-ColorModel");
  if (option != NULL)
  ...
  /* cupsICCQualifier2 */
  option = bobgui_printer_option_set_lookup (set, "cups-OutputMode");
  if (option != NULL)
  ...
  /* cupsICCQualifier3 */
  option = bobgui_printer_option_set_lookup (set, "cups-Resolution");
  if (option != NULL)
...
static void
colord_update_device (BobguiPrinterCups *printer)
{
  ...
  /* generate a known ID */
  colord_device_id = g_strdup_printf ("cups-%s", bobgui_printer_get_name (BOBGUI_PRINTER (printer)));
...
static void
colord_printer_details_aquired_cb (BobguiPrinterCups *printer,
                                   gboolean success,
                                   gpointer user_data)
{
  /* refresh the device */
...
/**
 * bobgui_printer_cups_new:
 *
 * Creates a new #BobguiPrinterCups.
 *
 * Returns: a new #BobguiPrinterCups
 **/
BobguiPrinterCups *
bobgui_printer_cups_new (const char      *name,
                      ...
                      gpointer         colord_client)
{
  ...
  result = g_object_new (BOBGUI_TYPE_PRINTER_CUPS,
			 "name", name,
			 "backend", backend,
			 "is-virtual", FALSE,
			 "accepts-pdf", accepts_pdf,
                         NULL);
  printer = BOBGUI_PRINTER_CUPS (result);
...
#if CUPS_VERSION_MAJOR < 3
ppd_file_t *
bobgui_printer_cups_get_ppd (BobguiPrinterCups *printer)
{
  return printer->ppd_file;
}

const char *
bobgui_printer_cups_get_ppd_name (BobguiPrinterCups  *printer)
{
  const char *result;
  ...
  if (result == NULL)
    result = bobgui_printer_get_name (BOBGUI_PRINTER (printer));

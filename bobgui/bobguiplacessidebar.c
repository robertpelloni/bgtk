/* BobguiPlacesSidebar - sidebar widget for places in the filesystem
 *
...
#include "bobguibookmarksmanagerprivate.h"
...
#include "bobguitrashmonitorprivate.h"
 ...
 * BobguiPlacesSidebar:
 *
 * BobguiPlacesSidebar is a widget that displays a list of frequently-used places in the
 * file system:  the userâ€™s home directory, the userâ€™s bookmarks, and volumes and drives.
 * This widget is used as a sidebar in BobguiFileChooser and may be used by file managers
 ...
 * for a Clipart folder.  You can do this with bobgui_places_sidebar_add_shortcut().
 *
 * To make use of the places sidebar, an application at least needs to connect
 * to the BobguiPlacesSidebar::open-location signal.  This is emitted when the
 * user selects in the sidebar a location to open.  The application should also
 * call bobgui_places_sidebar_set_location() when it changes the currently-viewed
 * location.
 *
 * # CSS nodes
 *
 ...
 * DROP_STATE_NEW_BOOKMARK_ARMED, however, if the client of BobguiPlacesSidebar
...
struct _BobguiPlacesSidebar {
  BobguiWidget parent;

  BobguiWidget *swin;
  BobguiWidget *list_box;
  BobguiWidget *new_bookmark_row;

  BobguiBookmarksManager     *bookmarks_manager;
  ...
  BobguiWidget *rename_popover;
  BobguiWidget *rename_entry;
  BobguiWidget *rename_button;
  BobguiWidget *rename_error;
  char *rename_uri;

  gulong trash_monitor_changed_id;
  BobguiWidget *trash_row;

  /* DND */
  gboolean   dragging_over;
  BobguiWidget *drag_row;
  int drag_row_height;
  int drag_row_x;
  int drag_row_y;
  BobguiWidget *row_placeholder;
  DropState drop_state;

  /* volume mounting - delayed open process */
  BobguiPlacesOpenFlags go_to_after_mount_open_flags;
  GCancellable *cancellable;

  BobguiWidget *popover;
  BobguiSidebarRow *context_row;
...
struct _BobguiPlacesSidebarClass {
  ...
  void    (* open_location)          (BobguiPlacesSidebar   *sidebar,
                                      GFile              *location,
                                      BobguiPlacesOpenFlags  open_flags);
  void    (* show_error_message)     (BobguiPlacesSidebar   *sidebar,
                                      const char         *primary,
                                      const char         *secondary);
  GdkDragAction (* drag_action_requested)  (BobguiPlacesSidebar   *sidebar,
                                      GFile              *dest_file,
                                      GSList             *source_file_list);
  GdkDragAction (* drag_action_ask)  (BobguiPlacesSidebar   *sidebar,
                                      GdkDragAction       actions);
  void    (* drag_perform_drop)      (BobguiPlacesSidebar   *sidebar,
                                      GFile              *dest_file,
                                      GList              *source_file_list,
                                      GdkDragAction       action);
  void    (* show_enter_location)    (BobguiPlacesSidebar   *sidebar);

  void    (* show_other_locations_with_flags)   (BobguiPlacesSidebar   *sidebar,
                                                 BobguiPlacesOpenFlags  open_flags);
...
static gboolean eject_or_unmount_bookmark  (BobguiSidebarRow *row);
static gboolean eject_or_unmount_selection (BobguiPlacesSidebar *sidebar);
static void  check_unmount_and_eject       (GMount   *mount,
                                            GVolume  *volume,
                                            GDrive   *drive,
                                            gboolean *show_unmount,
                                            gboolean *show_eject);
static void on_row_pressed  (BobguiGestureClick *gesture,
                             int                   n_press,
                             double                x,
                             double                y,
                             BobguiSidebarRow        *row);
static void on_row_released (BobguiGestureClick *gesture,
                             int                   n_press,
                             double                x,
                             double                y,
                             BobguiSidebarRow        *row);
static void on_row_dragged  (BobguiGestureDrag *gesture,
                             double          x,
                             double          y,
                             BobguiSidebarRow  *row);

static void popup_menu_cb    (BobguiSidebarRow   *row);
static void long_press_cb    (BobguiGesture      *gesture,
                              double           x,
                              double           y,
                              BobguiPlacesSidebar *sidebar);
static void stop_drop_feedback (BobguiPlacesSidebar *sidebar);
static GMountOperation * get_mount_operation (BobguiPlacesSidebar *sidebar);
static GMountOperation * get_unmount_operation (BobguiPlacesSidebar *sidebar);


G_DEFINE_TYPE (BobguiPlacesSidebar, bobgui_places_sidebar, BOBGUI_TYPE_WIDGET);

static void
emit_open_location (BobguiPlacesSidebar   *sidebar,
                    GFile              *location,
                    BobguiPlacesOpenFlags  open_flags)
{
...
static void
emit_show_error_message (BobguiPlacesSidebar *sidebar,
                         const char       *primary,
                         const char       *secondary)
{
...
static void
emit_show_enter_location (BobguiPlacesSidebar *sidebar)
{
  g_signal_emit (sidebar, places_sidebar_signals[SHOW_ENTER_LOCATION], 0);
}

static void
emit_show_other_locations_with_flags (BobguiPlacesSidebar   *sidebar,
                                      BobguiPlacesOpenFlags  open_flags)
{
...
static void
emit_unmount_operation (BobguiPlacesSidebar *sidebar,
                        GMountOperation  *mount_op)
{
...
emit_drag_action_requested (BobguiPlacesSidebar *sidebar,
                            GFile            *dest_file,
                            GSList           *source_file_list)
{
...
static void
emit_drag_perform_drop (BobguiPlacesSidebar *sidebar,
                        GFile            *dest_file,
                        GSList           *source_file_list,
                        GdkDragAction     action)
{
...
static void
list_box_header_func (BobguiListBoxRow *row,
                      BobguiListBoxRow *before,
                      gpointer       user_data)
{
  BobguiPlacesSectionType row_section_type;
  BobguiPlacesSectionType before_section_type;
           ...
           const char                  *tooltip)
{
  ...
  if (show_unmount || show_eject)
    g_assert (place_type != BOBGUI_PLACES_BOOKMARK);

  show_eject_button = (show_unmount || show_eject);

  row = g_object_new (BOBGUI_TYPE_SIDEBAR_ROW,
                      "sidebar", sidebar,
  ...
  eject_button = bobgui_sidebar_row_get_eject_button (BOBGUI_SIDEBAR_ROW (row));
  ...
  bobgui_list_box_insert (BOBGUI_LIST_BOX (sidebar->list_box), BOBGUI_WIDGET (row), -1);
...
static gboolean
recent_files_setting_is_enabled (BobguiPlacesSidebar *sidebar)
{
  BobguiSettings *settings;
  gboolean enabled;

  settings = bobgui_widget_get_settings (BOBGUI_WIDGET (sidebar));
  g_object_get (settings, "bobgui-recent-files-enabled", &enabled, NULL);
...
static gboolean
should_show_recent (BobguiPlacesSidebar *sidebar)
{
...
static void
open_home (BobguiPlacesSidebar *sidebar)
{
  const char *home_path;
  GFile       *home_dir;
...
static void
add_special_dirs (BobguiPlacesSidebar *sidebar)
{
  GList *dirs;
  int index;
  ...
  for (index = 0; index < G_USER_N_DIRECTORIES; index++)
    {
      ...
      name = _bobgui_bookmarks_manager_get_bookmark_label (sidebar->bookmarks_manager, root);
      if (!name)
...
static gboolean
file_is_shown (BobguiPlacesSidebar *sidebar,
               GFile            *file)
{
  char *uri;
  BobguiWidget *row;
  ...
  for (row = bobgui_widget_get_first_child (BOBGUI_WIDGET (sidebar->list_box));
       row != NULL && !found;
                                 ...
                                 gpointer      data)
{
  ...
  BobguiPlacesSidebar *sidebar = sdata->sidebar;
...
static void
add_application_shortcuts (BobguiPlacesSidebar *sidebar)
{
  guint i, n;
                                 ...
                                 gpointer      data)
{
  ...
  BobguiPlacesSidebar *sidebar = clos->sidebar;
  ...
  bookmark_name = _bobgui_bookmarks_manager_get_bookmark_label (sidebar->bookmarks_manager, root);
...
static void
update_trash_icon (BobguiPlacesSidebar *sidebar)
{
                    ...
                    gpointer    user_data)
{
    ...
    BobguiPlacesSidebar *sidebar = BOBGUI_PLACES_SIDEBAR (user_data);
...
static void
update_places (BobguiPlacesSidebar *sidebar)
{
  GList *mounts, *l, *ll;
  GMount *mount;
  GList *drives;
  ...
  while ((child = bobgui_widget_get_first_child (BOBGUI_WIDGET (sidebar->list_box))))
    bobgui_list_box_remove (BOBGUI_LIST_BOX (sidebar->list_box), child);
          ...
          for (ll = volumes; ll != NULL; ll = ll->next)
            {
              ...
              if (mount != NULL)
              ...
              else
                {
                  ...
                  tooltip = g_strdup_printf (_("Mount and open â€œ%sâ€"), name);
  ...
  /* add bookmarks */
  bookmarks = _bobgui_bookmarks_manager_list_bookmarks (sidebar->bookmarks_manager);
  ...
  for (sl = bookmarks, index = 0; sl; sl = sl->next, index++)
    {
      ...
      if (_bobgui_bookmarks_manager_get_is_builtin (sidebar->bookmarks_manager, root))
        continue;
  ...
  bobgui_widget_add_css_class (sidebar->new_bookmark_row, "sidebar-new-bookmark-row");
  ...
  bobgui_widget_set_visible (BOBGUI_WIDGET (sidebar), TRUE);
  /* We want this hidden by default, but need to do it after the show_all call */
  bobgui_sidebar_row_hide (BOBGUI_SIDEBAR_ROW (sidebar->new_bookmark_row), TRUE);
  ...
  if (original_uri)
    {
      ...
      bobgui_places_sidebar_set_location (sidebar, restore);
...
static gboolean
check_valid_drop_target (BobguiPlacesSidebar *sidebar,
                         BobguiSidebarRow    *row,
                         const GValue     *value)
{
  BobguiPlacesPlaceType place_type;
  ...
  if (place_type == BOBGUI_PLACES_CONNECT_TO_SERVER)
    {
      g_free (uri);
      return FALSE;
    }

  if (place_type == BOBGUI_PLACES_DROP_FEEDBACK)
    {
      g_free (uri);
      return TRUE;
    }
  ...
  if (place_type == BOBGUI_PLACES_BUILT_IN)
    {
  ...
  /* Dragging a bookmark? */
  if (G_VALUE_HOLDS (value, BOBGUI_TYPE_SIDEBAR_ROW))
    {
      /* Don't allow reordering bookmarks into non-bookmark areas */
      valid = section_type == BOBGUI_PLACES_SECTION_BOOKMARKS;
    }
...
static void
update_possible_drop_targets (BobguiPlacesSidebar *sidebar,
                              const GValue     *value)
{
  BobguiWidget *row;
  ...
  for (row = bobgui_widget_get_first_child (BOBGUI_WIDGET (sidebar->list_box));
       ...
       row = bobgui_widget_get_next_sibling (row))
    {
      ...
      sensitive = value == NULL ||
                  check_valid_drop_target (sidebar, BOBGUI_SIDEBAR_ROW (row), value);
      bobgui_widget_set_sensitive (row, sensitive);
    }
}

static void
start_drop_feedback (BobguiPlacesSidebar *sidebar,
                     const GValue     *value)
{
  if (value && !G_VALUE_HOLDS (value, BOBGUI_TYPE_SIDEBAR_ROW))
    {
      bobgui_sidebar_row_reveal (BOBGUI_SIDEBAR_ROW (sidebar->new_bookmark_row));
...
static void
stop_drop_feedback (BobguiPlacesSidebar *sidebar)
{
  ...
  if (sidebar->drop_state != DROP_STATE_NEW_BOOKMARK_ARMED_PERMANENT &&
    ...
    {
      bobgui_sidebar_row_hide (BOBGUI_SIDEBAR_ROW (sidebar->new_bookmark_row), FALSE);
      sidebar->drop_state = DROP_STATE_NORMAL;
    }

  if (sidebar->drag_row != NULL)
    {
      bobgui_widget_set_visible (sidebar->drag_row, TRUE);
      sidebar->drag_row = NULL;
    }

  if (sidebar->row_placeholder != NULL)
    {
      if (bobgui_widget_get_parent (sidebar->row_placeholder) != NULL)
        bobgui_list_box_remove (BOBGUI_LIST_BOX (sidebar->list_box), sidebar->row_placeholder);
...
static BobguiWidget *
create_placeholder_row (BobguiPlacesSidebar *sidebar)
{
  return g_object_new (BOBGUI_TYPE_SIDEBAR_ROW, "placeholder", TRUE, NULL);
}

static GdkDragAction
drag_motion_callback (BobguiDropTarget    *target,
                      double            x,
                      double            y,
                      BobguiPlacesSidebar *sidebar)
{
  ...
  action = 0;
  row = bobgui_list_box_get_row_at_y (BOBGUI_LIST_BOX (sidebar->list_box), y);

  bobgui_list_box_drag_unhighlight_row (BOBGUI_LIST_BOX (sidebar->list_box));
  ...
  if (!check_valid_drop_target (sidebar, BOBGUI_SIDEBAR_ROW (row), value))
    goto out;
  ...
  if (G_VALUE_HOLDS (value, BOBGUI_TYPE_SIDEBAR_ROW))
    {
      ...
      else if (BOBGUI_WIDGET (row) == sidebar->row_placeholder)
        {
          goto out;
        }
      ...
      if (row != NULL)
        {
          ...
          if (!bobgui_widget_compute_point (BOBGUI_WIDGET (sidebar), BOBGUI_WIDGET (row),
      ...
      bobgui_list_box_prepend (BOBGUI_LIST_BOX (sidebar->list_box),
                            sidebar->row_placeholder);
    }
  else if (G_VALUE_HOLDS (value, GDK_TYPE_FILE_LIST))
    {
      bobgui_list_box_drag_highlight_row (BOBGUI_LIST_BOX (sidebar->list_box), row);
...
static void
reorder_bookmarks (BobguiPlacesSidebar *sidebar,
                   BobguiSidebarRow    *row,
                   int               new_position)
{
  char *uri;
  ...
  _bobgui_bookmarks_manager_reorder_bookmark (sidebar->bookmarks_manager, file, new_position, NULL);
...
static void
drop_files_as_bookmarks (BobguiPlacesSidebar *sidebar,
                         GSList           *files,
                         int               position)
{
  GSList *l;
      ...
      if (info)
        {
          if (_bobgui_file_info_consider_as_directory (info))
            _bobgui_bookmarks_manager_insert_bookmark (sidebar->bookmarks_manager, f, position++, NULL);
...
static gboolean
drag_drop_callback (BobguiDropTarget    *target,
                    const GValue     *value,
                    double            x,
                    double            y,
                    BobguiPlacesSidebar *sidebar)
{
  ...
  BobguiPlacesPlaceType target_place_type;
  BobguiPlacesSectionType target_section_type;
  char *target_uri;
  BobguiListBoxRow *target_row;
  gboolean result;

  target_row = bobgui_list_box_get_row_at_y (BOBGUI_LIST_BOX (sidebar->list_box), y);
  if (target_row == NULL)
    return FALSE;

  if (!check_valid_drop_target (sidebar, BOBGUI_SIDEBAR_ROW (target_row), value))
    return FALSE;
  ...
  if (G_VALUE_HOLDS (value, BOBGUI_TYPE_SIDEBAR_ROW))
    {
      BobguiWidget *source_row;
      /* A bookmark got reordered */
      if (target_section_type != BOBGUI_PLACES_SECTION_BOOKMARKS)
        goto out;
    ...
    {
      /* Dropping URIs! */
      if (target_place_type == BOBGUI_PLACES_DROP_FEEDBACK)
        {
      ...
      else
        {
          ...
          emit_drag_perform_drop (sidebar,
                                  ...
                                  gdk_drop_get_actions (bobgui_drop_target_get_current_drop (target)));
 ...
 * bobgui_places_sidebar_show_drop_hints) and we will free the drag data next time
 * we build new drag data in drag_data_received.
 */
static void
drag_leave_callback (BobguiDropTarget *dest,
                     gpointer       user_data)
{
  BobguiPlacesSidebar *sidebar = BOBGUI_PLACES_SIDEBAR (user_data);
                              ...
                              gpointer      user_data)
{
  ...
  sidebar = BOBGUI_PLACES_SIDEBAR (user_data);
      ...
      if (error->code != G_IO_ERROR_FAILED_HANDLED)
        {
          ...
          primary = g_strdup_printf (_("Unable to start â€œ%sâ€"), name);
          g_free (name);
      ...
      if (error->code != G_IO_ERROR_FAILED_HANDLED &&
          error->code != G_IO_ERROR_ALREADY_MOUNTED)
        {
          ...
          if (g_str_has_prefix (error->message, "Error unlocking"))
             ...
             */
            primary = g_strdup_printf (_("Error unlocking â€œ%sâ€"), name);
          else
            primary = g_strdup_printf (_("Unable to access â€œ%sâ€"), name);
          g_free (name);
          emit_show_error_message (sidebar, primary, error->message);
          g_free (primary);
        }
...
static void
open_uri (BobguiPlacesSidebar   *sidebar,
          const char         *uri,
          BobguiPlacesOpenFlags  open_flags)
{
  GFile *location;
          ...
          BobguiPlacesOpenFlags  open_flags)
{
  ...
  if (place_type == BOBGUI_PLACES_OTHER_LOCATIONS)
    {
  ...
  else if (place_type == BOBGUI_PLACES_ENTER_LOCATION)
    {
...
open_shortcut_cb (GSimpleAction *action,
                  ...
                  gpointer       data)
{
  BobguiPlacesSidebar *sidebar = data;
  BobguiPlacesOpenFlags flags;

  flags = (BobguiPlacesOpenFlags)g_variant_get_int32 (parameter);
                 ...
                 gpointer       data)
{
  ...
  if (uri != NULL)
    {
      location = g_file_new_for_uri (uri);
      if (_bobgui_bookmarks_manager_insert_bookmark (sidebar->bookmarks_manager, location, -1, NULL))
        _bobgui_bookmarks_manager_set_bookmark_label (sidebar->bookmarks_manager, location, name, NULL);
      g_object_unref (location);
    }
...
static void
rename_entry_changed (BobguiEntry         *entry,
                      BobguiPlacesSidebar *sidebar)
{
  BobguiPlacesPlaceType type;
  char *name;
  char *uri;
  const char *new_name;
  gboolean found = FALSE;
  ...
  new_name = bobgui_editable_get_text (BOBGUI_EDITABLE (sidebar->rename_entry));

  if (strcmp (new_name, "") == 0)
    {
      bobgui_widget_set_sensitive (sidebar->rename_button, FALSE);
      bobgui_label_set_label (BOBGUI_LABEL (sidebar->rename_error), "");
      return;
    }
  ...
  for (row = bobgui_widget_get_first_child (BOBGUI_WIDGET (sidebar->list_box));
       ...
       row = bobgui_widget_get_next_sibling (row))
    {
      ...
      if ((type == BOBGUI_PLACES_XDG_DIR || type == BOBGUI_PLACES_BOOKMARK) &&
  ...
  bobgui_widget_set_sensitive (sidebar->rename_button, !found);
  bobgui_label_set_label (BOBGUI_LABEL (sidebar->rename_error),
                       found ? _("This name is already taken") : "");
}

static void
do_rename (BobguiButton        *button,
           BobguiPlacesSidebar *sidebar)
{
  char *new_text;
  GFile *file;

  new_text = g_strdup (bobgui_editable_get_text (BOBGUI_EDITABLE (sidebar->rename_entry)));

  file = g_file_new_for_uri (sidebar->rename_uri);
  if (!_bobgui_bookmarks_manager_has_bookmark (sidebar->bookmarks_manager, file))
    _bobgui_bookmarks_manager_insert_bookmark (sidebar->bookmarks_manager, file, -1, NULL);

  _bobgui_bookmarks_manager_set_bookmark_label (sidebar->bookmarks_manager, file, new_text, NULL);
  ...
  if (sidebar->rename_popover)
    bobgui_popover_popdown (BOBGUI_POPOVER (sidebar->rename_popover));
}

static void
on_rename_popover_destroy (BobguiWidget        *rename_popover,
                           BobguiPlacesSidebar *sidebar)
{
  if (sidebar)
    {
...
static void
create_rename_popover (BobguiPlacesSidebar *sidebar)
{
  BobguiWidget *popover;
  BobguiWidget *grid;
  BobguiWidget *label;
  ...
  bobgui_widget_set_parent (popover, BOBGUI_WIDGET (sidebar));
  ...
  bobgui_popover_set_position (BOBGUI_POPOVER (popover), BOBGUI_POS_RIGHT);
  grid = bobgui_grid_new ();
  bobgui_popover_set_child (BOBGUI_POPOVER (popover), grid);
  g_object_set (grid,
                "margin-start", 10,
                "margin-end", 10,
                "margin-top", 10,
  ...
  g_free (str);
  button = bobgui_button_new_with_mnemonic (_("_Rename"));
  bobgui_widget_add_css_class (button, "suggested-action");
  g_signal_connect (button, "clicked", G_CALLBACK (do_rename), sidebar);
  error = bobgui_label_new ("");
  bobgui_widget_set_halign (error, BOBGUI_ALIGN_START);
  bobgui_grid_attach (BOBGUI_GRID (grid), label, 0, 0, 2, 1);
  bobgui_grid_attach (BOBGUI_GRID (grid), entry, 0, 1, 1, 1);
  bobgui_grid_attach (BOBGUI_GRID (grid), button,1, 1, 1, 1);
  bobgui_grid_attach (BOBGUI_GRID (grid), error, 0, 2, 2, 1);
  bobgui_popover_set_default_widget (BOBGUI_POPOVER (popover), button);
 ...
 */
static void
update_popover_shadowing (BobguiWidget *row,
                          gboolean   shown)
{
  int count;
  ...
  if (count > 0)
    bobgui_widget_add_css_class (row, "has-open-popup");
  else
    bobgui_widget_remove_css_class (row, "has-open-popup");
}

static void
set_prelight (BobguiPlacesSidebar *sidebar)
{
  update_popover_shadowing (BOBGUI_WIDGET (sidebar->context_row), TRUE);
}

static void
unset_prelight (BobguiPlacesSidebar *sidebar)
{
  update_popover_shadowing (BOBGUI_WIDGET (sidebar->context_row), FALSE);
}

static void
setup_popover_shadowing (BobguiWidget        *popover,
                         BobguiPlacesSidebar *sidebar)
{
...
static void
show_rename_popover (BobguiSidebarRow *row)
{
  char *name;
  char *uri;
  BobguiPlacesSidebar *sidebar;
  ...
  bobgui_editable_set_text (BOBGUI_EDITABLE (sidebar->rename_entry), name);

  _popover_set_pointing_to_widget (BOBGUI_POPOVER (sidebar->rename_popover),
                                   BOBGUI_WIDGET (row));

  setup_popover_shadowing (sidebar->rename_popover, sidebar);

  bobgui_popover_popup (BOBGUI_POPOVER (sidebar->rename_popover));
  bobgui_widget_grab_focus (sidebar->rename_entry);
...
static void
rename_bookmark (BobguiSidebarRow *row)
{
  BobguiPlacesPlaceType type;

  g_object_get (row, "place-type", &type, NULL);

  if (type != BOBGUI_PLACES_BOOKMARK && type != BOBGUI_PLACES_XDG_DIR)
    return;
...
static void
remove_bookmark (BobguiSidebarRow *row)
{
  BobguiPlacesPlaceType type;
  char *uri;
  GFile *file;
  ...
  if (type == BOBGUI_PLACES_BOOKMARK)
    {
      file = g_file_new_for_uri (uri);
      _bobgui_bookmarks_manager_remove_bookmark (sidebar->bookmarks_manager, file, NULL);
      g_object_unref (file);
    }
      ...
      if (error->code != G_IO_ERROR_FAILED_HANDLED)
        {
          ...
          name = g_mount_get_name (mount);
          primary = g_strdup_printf (_("Unable to unmount â€œ%sâ€"), name);
          g_free (name);
          emit_show_error_message (sidebar, primary, error->message);
          g_free (primary);
        }
...
get_mount_operation (BobguiPlacesSidebar *sidebar)
{
  GMountOperation *mount_op;

  mount_op = bobgui_mount_operation_new (BOBGUI_WINDOW (bobgui_widget_get_root (BOBGUI_WIDGET (sidebar))));
...
static GMountOperation *
get_unmount_operation (BobguiPlacesSidebar *sidebar)
{
  GMountOperation *mount_op;
...
static gboolean
is_current_location_on_volume (BobguiPlacesSidebar *sidebar,
                               GMount           *mount,
                               GVolume          *volume,
                               GDrive           *drive)
{
  ...
  if (!g_drive_stop_finish (G_DRIVE (source_object), res, &error))
    {
      if (error->code != G_IO_ERROR_FAILED_HANDLED)
        {
          name = g_drive_get_name (G_DRIVE (source_object));
          primary = g_strdup_printf (_("Unable to stop â€œ%sâ€"), name);
          g_free (name);
          emit_show_error_message (sidebar, primary, error->message);
          g_free (primary);
        }
  ...
  if (!g_drive_eject_with_operation_finish (G_DRIVE (source_object), res, &error))
    {
      if (error->code != G_IO_ERROR_FAILED_HANDLED)
        {
          name = g_drive_get_name (G_DRIVE (source_object));
          primary = g_strdup_printf (_("Unable to eject â€œ%sâ€"), name);
          g_free (name);
          emit_show_error_message (sidebar, primary, error->message);
          g_free (primary);
        }
...
static gboolean
eject_or_unmount_bookmark (BobguiSidebarRow *row)
{
...
static gboolean
eject_or_unmount_selection (BobguiPlacesSidebar *sidebar)
{
  gboolean ret;
  BobguiListBoxRow *row;

  row = bobgui_list_box_get_selected_row (BOBGUI_LIST_BOX (sidebar->list_box));
  ret = eject_or_unmount_bookmark (BOBGUI_SIDEBAR_ROW (row));
  ...
  if (!g_drive_poll_for_media_finish (G_DRIVE (source_object), res, &error))
    {
      if (error->code != G_IO_ERROR_FAILED_HANDLED)
        {
          name = g_drive_get_name (G_DRIVE (source_object));
          primary = g_strdup_printf (_("Unable to poll â€œ%sâ€ for media changes"), name);
          g_free (name);
          emit_show_error_message (sidebar, primary, error->message);
          g_free (primary);
        }
...
static gboolean
on_key_pressed (BobguiEventControllerKey *controller,
                guint                  keyval,
                guint                  keycode,
  ...
  if (row)
    {
      ...
      if (keyval == GDK_KEY_Return ||
        ...
        {
          BobguiPlacesOpenFlags open_flags = BOBGUI_PLACES_OPEN_NORMAL;

          if ((state & modifiers) == GDK_SHIFT_MASK)
            open_flags = BOBGUI_PLACES_OPEN_NEW_TAB;
          else if ((state & modifiers) == GDK_CONTROL_MASK)
            open_flags = BOBGUI_PLACES_OPEN_NEW_WINDOW;

          open_row (BOBGUI_SIDEBAR_ROW (row), open_flags);
      ...
      if ((keyval == GDK_KEY_Delete ||
          ...
          (state & modifiers) == 0)
        {
          remove_bookmark (BOBGUI_SIDEBAR_ROW (row));
          return TRUE;
        }
      ...
      if ((keyval == GDK_KEY_F2) &&
          (state & modifiers) == 0)
        {
          rename_bookmark (BOBGUI_SIDEBAR_ROW (row));
          return TRUE;
        }
      ...
      if ((keyval == GDK_KEY_Menu) ||
        ...
        {
          popup_menu_cb (BOBGUI_SIDEBAR_ROW (row));
          return TRUE;
        }
    }
...
static void
on_row_popover_destroy (BobguiWidget        *row_popover,
                        BobguiPlacesSidebar *sidebar)
{
  if (sidebar)
  ...
  if (cloud_provider_account)
    {
      ...
      if (sidebar->open_flags & BOBGUI_PLACES_OPEN_NEW_TAB)
        {
          item = g_menu_item_new (_("Open in New _Tab"), "row.open-other");
          g_menu_item_set_action_and_target_value (item, "row.open-other", g_variant_new_int32(BOBGUI_PLACES_OPEN_NEW_TAB));
          g_menu_append_item (menu, item);
        }
      if (sidebar->open_flags & BOBGUI_PLACES_OPEN_NEW_WINDOW)
        {
      ...
      if (sidebar->popover)
        bobgui_widget_unparent (sidebar->popover);
...
static void
create_row_popover (BobguiPlacesSidebar *sidebar,
                    BobguiSidebarRow    *row)
{
  BobguiPlacesPlaceType type;
  GMenu *menu, *section;
  ...
  g_simple_action_set_enabled (G_SIMPLE_ACTION (action), (type == BOBGUI_PLACES_BOOKMARK));
  action = g_action_map_lookup_action (G_ACTION_MAP (sidebar->row_actions), "rename");
  g_simple_action_set_enabled (G_SIMPLE_ACTION (action), (type == BOBGUI_PLACES_BOOKMARK ||
                                                          type == BOBGUI_PLACES_XDG_DIR));
  action = g_action_map_lookup_action (G_ACTION_MAP (sidebar->row_actions), "open");
  g_simple_action_set_enabled (G_SIMPLE_ACTION (action), !bobgui_list_box_row_is_selected (BOBGUI_LIST_BOX_ROW (row)));
  ...
  if (sidebar->open_flags & BOBGUI_PLACES_OPEN_NEW_TAB)
    {
      ...
      g_menu_item_set_action_and_target_value (item, "row.open",
                                               g_variant_new_int32 (BOBGUI_PLACES_OPEN_NEW_TAB));
      g_menu_append_item (section, item);
      g_object_unref (item);
    }
  ...
  if (sidebar->open_flags & BOBGUI_PLACES_OPEN_NEW_WINDOW)
    {
      ...
      g_menu_item_set_action_and_target_value (item, "row.open",
                                               g_variant_new_int32 (BOBGUI_PLACES_OPEN_NEW_WINDOW));
      g_menu_append_item (section, item);
      g_object_unref (item);
    }
...
static void
show_row_popover (BobguiSidebarRow *row)
{
  BobguiPlacesSidebar *sidebar;

  g_object_get (row, "sidebar", &sidebar, NULL);

  g_clear_pointer (&sidebar->popover, bobgui_widget_unparent);

  create_row_popover (sidebar, row);

  _popover_set_pointing_to_widget (BOBGUI_POPOVER (sidebar->popover), BOBGUI_WIDGET (row));

  sidebar->context_row = row;
  bobgui_popover_popup (BOBGUI_POPOVER (sidebar->popover));

  g_object_unref (sidebar);
}

static void
on_row_activated (BobguiListBox    *list_box,
                  BobguiListBoxRow *row,
                  gpointer       user_data)
{
  BobguiSidebarRow *selected_row;
   ...
   */
  if (BOBGUI_PLACES_SIDEBAR (user_data)->dragging_over)
    return;

  selected_row = BOBGUI_SIDEBAR_ROW (bobgui_list_box_get_selected_row (list_box));
  open_row (selected_row, 0);
}

static void
on_row_pressed (BobguiGestureClick *gesture,
                int              n_press,
                double           x,
                double           y,
                BobguiSidebarRow   *row)
{
  BobguiPlacesSidebar *sidebar;
  BobguiPlacesSectionType section_type;
  BobguiPlacesPlaceType row_type;
...
static void
on_row_released (BobguiGestureClick *gesture,
                 int              n_press,
                 double           x,
                 double           y,
                 BobguiSidebarRow   *row)
      ...
      if (button == 2)
        {
          ...
          open_flags = (state & GDK_CONTROL_MASK) ?
            BOBGUI_PLACES_OPEN_NEW_WINDOW :
            BOBGUI_PLACES_OPEN_NEW_TAB;
        ...
        {
          if (row_type != BOBGUI_PLACES_CONNECT_TO_SERVER)
            show_row_popover (BOBGUI_SIDEBAR_ROW (row));
        }
    }
}

static void
on_row_dragged (BobguiGestureDrag *gesture,
                double          x,
                double          y,
                BobguiSidebarRow  *row)
{
  ...
  if (bobgui_drag_check_threshold_double (BOBGUI_WIDGET (row), 0, 0, x, y))
    {
      ...
      GdkSurface *surface;
      GdkDevice *device;
      BobguiWidget *drag_widget;
      GdkDrag *drag;

      bobgui_gesture_drag_get_start_point (gesture, &start_x, &start_y);
      if (!bobgui_widget_compute_point (BOBGUI_WIDGET (row),
                                     BOBGUI_WIDGET (sidebar),
      ...
      content = gdk_content_provider_new_typed (BOBGUI_TYPE_SIDEBAR_ROW, sidebar->drag_row);

      surface = bobgui_native_get_surface (bobgui_widget_get_native (BOBGUI_WIDGET (sidebar)));
      ...
      bobgui_widget_set_visible (sidebar->drag_row, FALSE);

      drag_widget = BOBGUI_WIDGET (bobgui_sidebar_row_clone (BOBGUI_SIDEBAR_ROW (sidebar->drag_row)));
      sidebar->drag_row_height = bobgui_widget_get_height (sidebar->drag_row);
      bobgui_widget_set_size_request (drag_widget,
                                   bobgui_widget_get_width (sidebar->drag_row),
                                   bobgui_widget_get_height (sidebar->drag_row));
      bobgui_widget_set_opacity (drag_widget, 0.8);

      bobgui_drag_icon_set_child (BOBGUI_DRAG_ICON (bobgui_drag_icon_get_for_drag (drag)), drag_widget);
...
static void
popup_menu_cb (BobguiSidebarRow *row)
{
  BobguiPlacesPlaceType row_type;
...
static void
long_press_cb (BobguiGesture       *gesture,
               double            x,
               double            y,
               BobguiPlacesSidebar *sidebar)
{
  ...
  row = BOBGUI_WIDGET (bobgui_list_box_get_row_at_y (BOBGUI_LIST_BOX (sidebar->list_box), y));
  if (BOBGUI_IS_SIDEBAR_ROW (row))
    popup_menu_cb (BOBGUI_SIDEBAR_ROW (row));
}

static int
list_box_sort_func (BobguiListBoxRow *row1,
                    BobguiListBoxRow *row2,
                    gpointer       user_data)
{
  BobguiPlacesSectionType section_type_1, section_type_2;
  BobguiPlacesPlaceType place_type_1, place_type_2;
  ...
  if (place_type_1 == BOBGUI_PLACES_CONNECT_TO_SERVER)
    {
      retval = 1;
    }
  else if (place_type_2 == BOBGUI_PLACES_CONNECT_TO_SERVER)
    {
      retval = -1;
    }
  else
    {
      if (section_type_1 == section_type_2)
        {
          if ((section_type_1 == BOBGUI_PLACES_SECTION_COMPUTER &&
               place_type_1 == place_type_2 &&
               place_type_1 == BOBGUI_PLACES_XDG_DIR) ||
              section_type_1 == BOBGUI_PLACES_SECTION_MOUNTS)
            {
              retval = g_utf8_collate (label_1, label_2);
            }
          else if ((place_type_1 == BOBGUI_PLACES_BOOKMARK || place_type_2 == BOBGUI_PLACES_DROP_FEEDBACK) &&
                   (place_type_1 == BOBGUI_PLACES_DROP_FEEDBACK || place_type_2 == BOBGUI_PLACES_BOOKMARK))
            {
           ...
           * of the row, we need to increase the order-index.
           */
          else if (place_type_1 == BOBGUI_PLACES_BOOKMARK_PLACEHOLDER && place_type_2 == BOBGUI_PLACES_BOOKMARK)
            {
              if (index_1 == index_2)
          ...
          else if (place_type_1 == BOBGUI_PLACES_BOOKMARK && place_type_2 == BOBGUI_PLACES_BOOKMARK_PLACEHOLDER)
            {
              if (index_1 == index_2)
                retval =  index_1 - index_2 + 1;
              else
...
static void
update_hostname (BobguiPlacesSidebar *sidebar)
{
  GVariant *variant;
  gsize len;
  const char *hostname;
...
hostname_proxy_new_cb (GObject      *source_object,
                       ...
                       gpointer      user_data)
{
  BobguiPlacesSidebar *sidebar = user_data;
  GError *error = NULL;
  GDBusProxy *proxy;
...
static void
create_volume_monitor (BobguiPlacesSidebar *sidebar)
{
...
static void
shell_shows_desktop_changed (BobguiSettings *settings,
                             GParamSpec  *pspec,
                             gpointer     user_data)
{
  BobguiPlacesSidebar *sidebar = user_data;
  ...
  g_assert (settings == sidebar->bobgui_settings);

  /* Check if the user explicitly set this and, if so, don't change it. */
  if (sidebar->show_desktop_set)
    return;

  g_object_get (settings, "bobgui-shell-shows-desktop", &show_desktop, NULL);
...
static void
bobgui_places_sidebar_init (BobguiPlacesSidebar *sidebar)
{
  ...
  sidebar->open_flags = BOBGUI_PLACES_OPEN_NORMAL;

  sidebar->bookmarks_manager = _bobgui_bookmarks_manager_new ((BobguiBookmarksChangedFunc)update_places, sidebar);

  sidebar->trash_monitor = _bobgui_trash_monitor_get ();
  ...
  sidebar->swin = bobgui_scrolled_window_new ();
  bobgui_widget_set_parent (sidebar->swin, BOBGUI_WIDGET (sidebar));
  bobgui_widget_set_size_request (sidebar->swin, 140, 280);

  bobgui_scrolled_window_set_policy (BOBGUI_SCROLLED_WINDOW (sidebar->swin),
                                  BOBGUI_POLICY_NEVER,
                                  BOBGUI_POLICY_AUTOMATIC);

  bobgui_widget_add_css_class (BOBGUI_WIDGET (sidebar), "sidebar");

  /* list box */
  sidebar->list_box = bobgui_list_box_new ();
  bobgui_widget_add_css_class (sidebar->list_box, "navigation-sidebar");

  bobgui_list_box_set_header_func (BOBGUI_LIST_BOX (sidebar->list_box),
                                list_box_header_func, sidebar, NULL);
  bobgui_list_box_set_sort_func (BOBGUI_LIST_BOX (sidebar->list_box),
                              list_box_sort_func, NULL, NULL);
  bobgui_list_box_set_selection_mode (BOBGUI_LIST_BOX (sidebar->list_box), BOBGUI_SELECTION_SINGLE);
  bobgui_list_box_set_activate_on_single_click (BOBGUI_LIST_BOX (sidebar->list_box), TRUE);
  bobgui_list_box_set_tab_behavior (BOBGUI_LIST_BOX (sidebar->list_box), BOBGUI_LIST_TAB_ITEM);
  ...
  bobgui_widget_add_controller (sidebar->list_box, controller);
  ...
  bobgui_widget_add_controller (BOBGUI_WIDGET (sidebar), BOBGUI_EVENT_CONTROLLER (gesture));

  /* DND support */
  target = bobgui_drop_target_new (G_TYPE_INVALID, GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);
  bobgui_drop_target_set_preload (target, TRUE);
  bobgui_drop_target_set_gtypes (target, (GType[2]) { BOBGUI_TYPE_SIDEBAR_ROW, GDK_TYPE_FILE_LIST }, 2);
  ...
  bobgui_widget_add_controller (sidebar->list_box, BOBGUI_EVENT_CONTROLLER (target));

  sidebar->drag_row = NULL;
  sidebar->row_placeholder = NULL;
  sidebar->dragging_over = FALSE;

  bobgui_scrolled_window_set_child (BOBGUI_SCROLLED_WINDOW (sidebar->swin), sidebar->list_box);
  ...
  sidebar->bobgui_settings = bobgui_settings_get_default ();
  g_signal_connect (sidebar->bobgui_settings, "notify::bobgui-shell-shows-desktop",
                    G_CALLBACK (shell_shows_desktop_changed), sidebar);
  g_object_get (sidebar->bobgui_settings, "bobgui-shell-shows-desktop", &show_desktop, NULL);
  sidebar->show_desktop = show_desktop;
  ...
  g_action_map_add_action_entries (G_ACTION_MAP (sidebar->row_actions),
                                   ...
                                   sidebar);
  bobgui_widget_insert_action_group (BOBGUI_WIDGET (sidebar), "row", sidebar->row_actions);
}

static void
bobgui_places_sidebar_set_property (GObject      *obj,
                                 guint         property_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  BobguiPlacesSidebar *sidebar = BOBGUI_PLACES_SIDEBAR (obj);

  switch (property_id)
    {
    case PROP_LOCATION:
      bobgui_places_sidebar_set_location (sidebar, g_value_get_object (value));
      break;

    case PROP_OPEN_FLAGS:
      bobgui_places_sidebar_set_open_flags (sidebar, g_value_get_flags (value));
      break;

    case PROP_SHOW_RECENT:
      bobgui_places_sidebar_set_show_recent (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_DESKTOP:
      bobgui_places_sidebar_set_show_desktop (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_ENTER_LOCATION:
      bobgui_places_sidebar_set_show_enter_location (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_OTHER_LOCATIONS:
      bobgui_places_sidebar_set_show_other_locations (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_TRASH:
      bobgui_places_sidebar_set_show_trash (sidebar, g_value_get_boolean (value));
      break;
...
static void
bobgui_places_sidebar_get_property (GObject    *obj,
                                 guint       property_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_LOCATION:
      g_value_take_object (value, bobgui_places_sidebar_get_location (sidebar));
      break;

    case PROP_OPEN_FLAGS:
      g_value_set_flags (value, bobgui_places_sidebar_get_open_flags (sidebar));
      break;

    case PROP_SHOW_RECENT:
      g_value_set_boolean (value, bobgui_places_sidebar_get_show_recent (sidebar));
      break;

    case PROP_SHOW_DESKTOP:
      g_value_set_boolean (value, bobgui_places_sidebar_get_show_desktop (sidebar));
      break;

    case PROP_SHOW_ENTER_LOCATION:
      g_value_set_boolean (value, bobgui_places_sidebar_get_show_enter_location (sidebar));
      break;

    case PROP_SHOW_OTHER_LOCATIONS:
      g_value_set_boolean (value, bobgui_places_sidebar_get_show_other_locations (sidebar));
      break;

    case PROP_SHOW_TRASH:
      g_value_set_boolean (value, bobgui_places_sidebar_get_show_trash (sidebar));
      break;
...
bobgui_places_sidebar_dispose (GObject *object)
{
  ...
  sidebar = BOBGUI_PLACES_SIDEBAR (object);
  ...
  if (sidebar->bookmarks_manager != NULL)
    {
      _bobgui_bookmarks_manager_free (sidebar->bookmarks_manager);
  ...
  if (sidebar->rename_popover)
    {
      bobgui_widget_unparent (sidebar->rename_popover);
      sidebar->rename_popover = NULL;
      sidebar->rename_entry = NULL;
      sidebar->rename_button = NULL;
      sidebar->rename_error = NULL;
  ...
  if (sidebar->bobgui_settings)
    {
      g_signal_handlers_disconnect_by_func (sidebar->bobgui_settings, shell_shows_desktop_changed, sidebar);
      sidebar->bobgui_settings = NULL;
    }
...
static void
bobgui_places_sidebar_measure (BobguiWidget      *widget,
...
{
  BobguiPlacesSidebar *sidebar = BOBGUI_PLACES_SIDEBAR (widget);

  bobgui_widget_measure (sidebar->swin,
                      orientation,
                      for_size,
                      minimum, natural,
...
static void
bobgui_places_sidebar_size_allocate (BobguiWidget *widget,
                                  int        width,
                                  int        height,
                                  int        baseline)
{
  ...
  bobgui_widget_size_allocate (sidebar->swin,
                            &(BobguiAllocation) { 0, 0, width, height },
                            baseline);

  if (sidebar->popover)
    bobgui_popover_present (BOBGUI_POPOVER (sidebar->popover));

  if (sidebar->rename_popover)
    bobgui_popover_present (BOBGUI_POPOVER (sidebar->rename_popover));
}
...
bobgui_places_sidebar_class_init (BobguiPlacesSidebarClass *class)
{
  ...
  /*
   * BobguiPlacesSidebar::open-location:
   * @sidebar: the object which received the signal.
   * @location: (type Gio.File): GFile to which the caller should switch.
   * @open_flags: a single value from BobguiPlacesOpenFlags specifying how the @location should be opened.
   *
  ...
  places_sidebar_signals [OPEN_LOCATION] =
          g_signal_new (I_("open-location"),
                        ...
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, open_location),
                        NULL, NULL,
                        _bobgui_marshal_VOID__OBJECT_FLAGS,
                        G_TYPE_NONE, 2,
                        G_TYPE_OBJECT, BOBGUI_TYPE_PLACES_OPEN_FLAGS);
  ...
  places_sidebar_signals [SHOW_ERROR_MESSAGE] =
          g_signal_new (I_("show-error-message"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, show_error_message),
                        NULL, NULL,
  ...
  /*
   * BobguiPlacesSidebar::show-enter-location:
   * @sidebar: the object which received the signal.
   *
   * The places sidebar emits this signal when it needs the calling
  ...
  places_sidebar_signals [SHOW_ENTER_LOCATION] =
          g_signal_new (I_("show-enter-location"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, show_enter_location),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 0);

  /*
   * BobguiPlacesSidebar::drag-action-requested:
   * @sidebar: the object which received the signal.
  ...
  places_sidebar_signals [DRAG_ACTION_REQUESTED] =
          g_signal_new (I_("drag-action-requested"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, drag_action_requested),
                        NULL, NULL,
                        _bobgui_marshal_INT__OBJECT_OBJECT_POINTER,
                        GDK_TYPE_DRAG_ACTION, 2,
                        G_TYPE_OBJECT,
                        GDK_TYPE_FILE_LIST);
  g_signal_set_va_marshaller (places_sidebar_signals[DRAG_ACTION_REQUESTED],
                              G_OBJECT_CLASS_TYPE (gobject_class),
                              _bobgui_marshal_INT__OBJECT_OBJECT_POINTERv);

  /*
   * BobguiPlacesSidebar::drag-action-ask:
   * @sidebar: the object which received the signal.
  ...
  places_sidebar_signals [DRAG_ACTION_ASK] =
          g_signal_new (I_("drag-action-ask"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, drag_action_ask),
                        NULL, NULL,
                        _bobgui_marshal_INT__INT,
                        GDK_TYPE_DRAG_ACTION, 1,
                        GDK_TYPE_DRAG_ACTION);
  g_signal_set_va_marshaller (places_sidebar_signals[DRAG_ACTION_ASK],
                              G_OBJECT_CLASS_TYPE (gobject_class),
                              _bobgui_marshal_INT__INTv);

  /*
   * BobguiPlacesSidebar::drag-perform-drop:
   * @sidebar: the object which received the signal.
  ...
  places_sidebar_signals [DRAG_PERFORM_DROP] =
          g_signal_new (I_("drag-perform-drop"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, drag_perform_drop),
                        NULL, NULL,
                        _bobgui_marshal_VOID__OBJECT_POINTER_INT,
                        G_TYPE_NONE, 3,
                        G_TYPE_OBJECT,
                        GDK_TYPE_FILE_LIST,
                        GDK_TYPE_DRAG_ACTION);
  g_signal_set_va_marshaller (places_sidebar_signals[DRAG_PERFORM_DROP],
                              G_OBJECT_CLASS_TYPE (gobject_class),
                              _bobgui_marshal_VOID__OBJECT_POINTER_INTv);

  /*
   * BobguiPlacesSidebar::show-other-locations-with-flags:
   * @sidebar: the object which received the signal.
   * @open_flags: a single value from BobguiPlacesOpenFlags specifying how it should be opened.
   *
   * The places sidebar emits this signal when it needs the calling
  ...
  places_sidebar_signals [SHOW_OTHER_LOCATIONS_WITH_FLAGS] =
          g_signal_new (I_("show-other-locations-with-flags"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, show_other_locations_with_flags),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 1,
   ...
   * In this way the application using the BobguiPlacesSidebar can track the
  ...
  places_sidebar_signals [MOUNT] =
          g_signal_new (I_("mount"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, mount),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE,
                        1,
  ...
  places_sidebar_signals [UNMOUNT] =
          g_signal_new (I_("unmount"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (BobguiPlacesSidebarClass, unmount),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE,
                        1,
  ...
  bobgui_widget_class_set_css_name (widget_class, I_("placessidebar"));
}

/*
 * bobgui_places_sidebar_new:
 *
 * Creates a new BobguiPlacesSidebar widget.
 *
 * The application should connect to at least the
 * BobguiPlacesSidebar::open-location signal to be notified
 * when the user makes a selection in the sidebar.
 *
 * Returns: a newly created BobguiPlacesSidebar
 */
BobguiWidget *
bobgui_places_sidebar_new (void)
{
  return BOBGUI_WIDGET (g_object_new (bobgui_places_sidebar_get_type (), NULL));
}
 ...
 * â€œdirectlyâ€ into their main view, while others may support opening locations
 * in a new notebook tab or a new window.
 *
 * This function is used to tell the places @sidebar about the ways in which the
 * application can open new locations, so that the sidebar can display (or not)
 * the â€œOpen in new tabâ€ and â€œOpen in new windowâ€ menu items as appropriate.
 *
 ...
 * to callbacks for the â€œopen-locationâ€ signal.
 */
void
...
/*
 * bobgui_places_sidebar_get_open_flags:
 * @sidebar: a BobguiPlacesSidebar
 *
 * Gets the open flags.
 *
 * Returns: the BobguiPlacesOpenFlags of @sidebar
 */
BobguiPlacesOpenFlags
bobgui_places_sidebar_get_open_flags (BobguiPlacesSidebar *sidebar)
{
  g_return_val_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar), 0);

  return sidebar->open_flags;
}

/*
 * bobgui_places_sidebar_set_location:
 * @sidebar: a places sidebar
 ...
 */
void
bobgui_places_sidebar_set_location (BobguiPlacesSidebar *sidebar,
                                 GFile            *location)
{
  BobguiWidget *row;
  char *row_uri;
  ...
  bobgui_list_box_unselect_all (BOBGUI_LIST_BOX (sidebar->list_box));
  ...
  for (row = bobgui_widget_get_first_child (BOBGUI_WIDGET (sidebar->list_box));
       ...
       row = bobgui_widget_get_next_sibling (row))
    {
      ...
      if (row_uri != NULL && g_strcmp0 (row_uri, uri) == 0)
        {
          bobgui_list_box_select_row (BOBGUI_LIST_BOX (sidebar->list_box),
                                   BOBGUI_LIST_BOX_ROW (row));
          found = TRUE;
        }
...
/*
 * bobgui_places_sidebar_get_location:
 * @sidebar: a places sidebar
 *
 * Gets the currently selected location in the @sidebar. This can be %NULL when
 * nothing is selected, for example, when bobgui_places_sidebar_set_location() has
 * been called with a location that is not among the sidebarâ€™s list of places to
 * show.
 *
 ...
 */
GFile *
bobgui_places_sidebar_get_location (BobguiPlacesSidebar *sidebar)
{
  BobguiListBoxRow *selected;
  GFile *file;
...
char *
bobgui_places_sidebar_get_location_title (BobguiPlacesSidebar *sidebar)
{
  BobguiListBoxRow *selected;
  char *title;
...
/*
 * bobgui_places_sidebar_set_show_recent:
 * @sidebar: a places sidebar
 ...
 */
void
bobgui_places_sidebar_set_show_recent (BobguiPlacesSidebar *sidebar,
...
/*
 * bobgui_places_sidebar_get_show_recent:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with bobgui_places_sidebar_set_show_recent()
 *
 * Returns: %TRUE if the sidebar will display a builtin shortcut for recent files
 */
gboolean
bobgui_places_sidebar_get_show_recent (BobguiPlacesSidebar *sidebar)
{
...
/*
 * bobgui_places_sidebar_set_show_desktop:
 * @sidebar: a places sidebar
 ...
 */
void
bobgui_places_sidebar_set_show_desktop (BobguiPlacesSidebar *sidebar,
                                     gboolean          show_desktop)
{
  g_return_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar));

  /* Don't bother disconnecting from the BobguiSettings -- it will just
...
/*
 * bobgui_places_sidebar_get_show_desktop:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with bobgui_places_sidebar_set_show_desktop()
 *
 * Returns: %TRUE if the sidebar will display a builtin shortcut to the desktop folder.
 */
gboolean
bobgui_places_sidebar_get_show_desktop (BobguiPlacesSidebar *sidebar)
{
  g_return_val_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
/*
 * bobgui_places_sidebar_set_show_enter_location:
 * @sidebar: a places sidebar
 ...
 * BobguiPlacesSidebar::show-enter-location signal.
 */
void
bobgui_places_sidebar_set_show_enter_location (BobguiPlacesSidebar *sidebar,
...
/*
 * bobgui_places_sidebar_get_show_enter_location:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with bobgui_places_sidebar_set_show_enter_location()
 *
 * Returns: %TRUE if the sidebar will display an â€œEnter Locationâ€ item.
 */
gboolean
...
/*
 * bobgui_places_sidebar_set_show_other_locations:
 * @sidebar: a places sidebar
 ...
 *
 * If you enable this, you should connect to the
 * BobguiPlacesSidebar::show-other-locations-with-flags signal.
 */
void
bobgui_places_sidebar_set_show_other_locations (BobguiPlacesSidebar *sidebar,
...
/*
 * bobgui_places_sidebar_get_show_other_locations:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with bobgui_places_sidebar_set_show_other_locations()
 *
 * Returns: %TRUE if the sidebar will display an â€œOther Locationsâ€ item.
 */
gboolean
...
/*
 * bobgui_places_sidebar_set_show_trash:
 * @sidebar: a places sidebar
 ...
 */
void
bobgui_places_sidebar_set_show_trash (BobguiPlacesSidebar *sidebar,
                                   gboolean          show_trash)
{
  g_return_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar));
...
/*
 * bobgui_places_sidebar_get_show_trash:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with bobgui_places_sidebar_set_show_trash()
 *
 * Returns: %TRUE if the sidebar will display a â€œTrashâ€ item.
 */
gboolean
bobgui_places_sidebar_get_show_trash (BobguiPlacesSidebar *sidebar)
{
  g_return_val_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar), TRUE);

  return sidebar->show_trash;
}

/*
 * bobgui_places_sidebar_add_shortcut:
 * @sidebar: a places sidebar
 * @location: location to add as an application-specific shortcut
 *
 * Applications may want to present some folders in the places sidebar if
 * they could be immediately useful to users.  For example, a drawing
 * program could add a â€œ/usr/share/clipartâ€ location when the sidebar is
 * being used in an â€œInsert Clipartâ€ dialog box.
 *
 * This function adds the specified @location to a special place for immutable
 * shortcuts.  The shortcuts are application-specific; they are not shared
 * across applications, and they are not persistent.  If this function
 * is called multiple times with different locations, then they are added
 * to the sidebarâ€™s list in the same order as the function is called.
 */
void
bobgui_places_sidebar_add_shortcut (BobguiPlacesSidebar *sidebar,
                                 GFile            *location)
{
  g_return_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar));
...
/*
 * bobgui_places_sidebar_remove_shortcut:
 * @sidebar: a places sidebar
 * @location: location to remove
 *
 * Removes an application-specific shortcut that has been previously been
 * inserted with bobgui_places_sidebar_add_shortcut().  If the @location is not a
 * shortcut in the sidebar, then nothing is done.
 */
void
bobgui_places_sidebar_remove_shortcut (BobguiPlacesSidebar *sidebar,
                                    GFile            *location)
{
  guint i, n;
...
/*
 * bobgui_places_sidebar_list_shortcuts:
 * @sidebar: a places sidebar
 *
 ...
 *   application-specific shortcuts with bobgui_places_sidebar_add_shortcut()
 */
GListModel *
bobgui_places_sidebar_get_shortcuts (BobguiPlacesSidebar *sidebar)
{
  g_return_val_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar), NULL);

  return G_LIST_MODEL (g_object_ref (sidebar->shortcuts));
}

/*
 * bobgui_places_sidebar_get_nth_bookmark:
 * @sidebar: a places sidebar
 ...
 * the â€œAlt-1â€, â€œAlt-2â€, etc. shortcuts, which activate the corresponding bookmark.
 *
 ...
 */
GFile *
bobgui_places_sidebar_get_nth_bookmark (BobguiPlacesSidebar *sidebar,
                                     int               n)
{
  BobguiWidget *row;
  int k;
  ...
  for (row = bobgui_widget_get_first_child (BOBGUI_WIDGET (sidebar->list_box));
       ...
       row = bobgui_widget_get_next_sibling (row))
    {
      ...
      g_object_get (row,
                    ...
                    "uri", &uri,
                    NULL);
      if (place_type == BOBGUI_PLACES_BOOKMARK)
        {
          if (k == n)
            {
              file = g_file_new_for_uri (uri);
...
/*
 * bobgui_places_sidebar_set_drop_targets_visible:
 * @sidebar: a places sidebar.
 * @visible: whether to show the valid targets or not.
 *
 * Make the BobguiPlacesSidebar show drop targets, so it can show the available
 ...
 * automatically if the drag finishes in the BobguiPlacesSidebar. You only need
 * to unset the state when the drag ends on some other widget on your application.
 */
void
bobgui_places_sidebar_set_drop_targets_visible (BobguiPlacesSidebar *sidebar,
                                             gboolean          visible)
{
  g_return_if_fail (BOBGUI_IS_PLACES_SIDEBAR (sidebar));
 ...
 */
void
bobgui_places_sidebar_set_show_starred_location (BobguiPlacesSidebar *sidebar,
 ...
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with bobgui_places_sidebar_set_show_starred_location()
 *
